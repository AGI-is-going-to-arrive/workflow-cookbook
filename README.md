...
Claude Code 悄然引入的 **Workflow** 特性（`CLAUDE_CODE_WORKFLOWS`），是一个用 JavaScript 脚本化编排多 Agent 的确定性引擎。它不是 MCP，不是 Skills，不是 Subagents，更不是 Agent Teams——它是一种全新的、**可复用、可测试、可共享**的工程流水线。

本书通过 **22 个章节 + 4 个附录**，从零到一带你：
...
| # | 章节 | 关键词 |
|---|------|--------|
| 01 | [Workflow 是什么](docs/zh/01-what-is-workflow.md) | 确定性编排引擎 / CLAUDE_CODE_WORKFLOWS |
| 02 | [核心概念全景图](docs/zh/02-core-concepts.md) | meta / phase / agent / parallel / pipeline / schema |
| 03 | [定位矩阵](docs/zh/03-positioning-matrix.md) | Workflow vs Subagents vs Agent Teams vs Skills |

### Part 2 · 基础篇 Foundations

| # | 章节 | 关键词 |
|---|------|--------|
| 04 | [环境搭建 & Hello Workflow](docs/zh/04-hello-workflow.md) | 第一个 Workflow / smoke test |
| 05 | [agent() 完全指南](docs/zh/05-agent-guide.md) | prompt / schema / model / isolation / agentType |
| 06 | [并发编排双刃剑](docs/zh/06-concurrency.md) | parallel() 屏障 vs pipeline() 流水线 |
| 07 | [结构化输出](docs/zh/07-structured-output.md) | JSON Schema / 数据流传递 |
| 08 | [进度监控与调试](docs/zh/08-monitoring.md) | phase() / log() / /workflows |

### Part 3 · 实战食谱 Recipes

| # | 章节 | 关键词 |
|---|------|--------|
| 09 | [分片代码审查](docs/zh/09-sharded-review.md) | 大代码库分治 + 对抗验证 |
| 10 | [PR 多角色 Review](docs/zh/10-multi-role-review.md) | 安全 / 性能 / 架构多维度 |
| 11 | [生成-批评-修复 (GCF Loop)](docs/zh/11-gcf-loop.md) | 循环退出条件设计 |
| 12 | [深度研究](docs/zh/12-deep-research.md) | 多源搜索 + 交叉验证 |
| 13 | [Prompt/Agent 评估](docs/zh/13-evaluation.md) | A/B 测试 + 评委面板 |
| 14 | [Bug 猎手](docs/zh/14-bug-hunter.md) | 自繁殖发现池 + 对抗验证 |

### Part 4 · 进阶篇 Advanced Patterns

| # | 章节 | 关键词 |
|---|------|--------|
| 15 | [预算控制与动态循环](docs/zh/15-budget-control.md) | budget.total / remaining / loop |
| 16 | [断点续传](docs/zh/16-resume.md) | resumeFromRunId / 缓存命中 |
| 17 | [Worktree 隔离](docs/zh/17-worktree.md) | 并行文件修改 |
| 18 | [嵌套 Workflow](docs/zh/18-nested-workflow.md) | workflow() 子流程 |
| 19 | [质量模式合集](docs/zh/19-quality-patterns.md) | 对抗验证 / 评委面板 / 循环到干 |

### Part 5 · 生态篇 Ecosystem

| # | 章节 | 关键词 |
|---|------|--------|
| 20 | [四大 Workflow 系统横评](docs/zh/20-ecosystem-review.md) | ccg / OMC / OmO / Superpowers |
| 21 | [精华提取术](docs/zh/21-pattern-extraction.md) | 解构 → 抽象 → 适配 → 验证 |
| 22 | [构建你的 Workflow 库](docs/zh/22-build-your-library.md) | 目录结构 / 命名 / 版本管理 |

### 附录 Reference

| # | 内容 |
|---|------|
| A | [API 完整参考](docs/zh/appendix-a-api-reference.md) |
| B | [常见问题与陷阱](docs/zh/appendix-b-faq.md) |
| C | [最佳实践清单](docs/zh/appendix-c-best-practices.md) |
| D | [术语表](docs/zh/appendix-d-glossary.md) |
| E | [示例脚本合集](examples/) |
...
- 网站使用纯 HTML/CSS/JS，零依赖，部署在 GitHub Pages
- 中英文完全对照，一键切换
- 代码示例可直接复制运行
