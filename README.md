...
| 10 | PR 多角色 Review *(撰写中)* | 安全 / 性能 / 架构多维度 |
| 11 | [生成-批评-修复 (GCF Loop)](#ch11) | 循环退出条件设计 |
| 12 | 深度研究 *(撰写中)* | 多源搜索 + 交叉验证 |
| 13 | Prompt/Agent 评估 *(撰写中)* | A/B 测试 + 评委面板 |
| 14 | Bug 猎手 *(撰写中)* | 自繁殖发现池 + 对抗验证 |
...
| 16 | 断点续传 *(撰写中)* | resumeFromRunId / 缓存命中 |
| 17 | Worktree 隔离 *(撰写中)* | 并行文件修改 |
| 18 | 嵌套 Workflow *(撰写中)* | workflow() 子流程 |
| 19 | 质量模式合集 *(撰写中)* | 对抗验证 / 评委面板 / 循环到干 |
