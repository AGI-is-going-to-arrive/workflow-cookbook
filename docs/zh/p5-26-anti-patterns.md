...
**后果：** 「脚本禁用 `Date.now()` / `Math.random()` / 无参 `new Date()`」（B 节硬约束），它们会「破坏可重放性 → 续传失效」（B 节注）。轻则抛错，重则就算不抛错，每跑一次结果都不一样，于是续传永远命不中缓存（第 22 章那条「未改动 agent 0 token/8ms」的红利你彻底吃不到），回归测试也退化成「每次全量重跑」。
