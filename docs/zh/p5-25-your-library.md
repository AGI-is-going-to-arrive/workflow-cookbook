...
> 从零开始写自己的工作流？先读[第 27 章《工作流创作流程》](#/zh/p6-27)，再回来把它沉淀成库。
>
...
## 25.1 建库的两条路：先官方台面，后脚本落盘

「建立一个自己的 workflow 库」听起来像个大工程，但官方给普通用户留的入口其实轻到不能再轻：**跑通一个满意的 run，按一个键，它就进库了**。先把两条路摆清楚，你才知道自己该走哪条。

**路一·官方台面（绝大多数人从这儿开始）。** 你让 Claude 跑了个 workflow，结果很满意，想以后一键复用，那就在 `/workflows` 视图里按 **`s`**，这次 run 背后的脚本就被**存成一条 `/` 命令**：它进你敲 `/` 时的**自动补全**列表，和官方自带的 `/deep-research` **并列**，下次直接 `/<你起的名字>` 就能再跑（官方原话：你保存的 workflow 会变成 `/` 命令、出现在自动补全里）。这条路**不用你碰任何文件、配任何目录**，按 `s` 存命令的台面操作在[《官方操作面板》§6](#/zh/p2-ops) 有照着就能上手的演示。对多数人来说，「库」就是这么一条条 `s` 攒出来的。

> **按键速记**：在 `/workflows` 里选中这次 run，按 `s`，用 `Tab` 切项目级 / 个人级，按 `Enter`。
> 完整的台面操作（每个按键、每个落点的细节）仍看[《官方操作面板》§6](#/zh/p2-ops)。

**路二·脚本落盘（power-user / 进阶）。** 等你攒多了、想正经管理（版本化、参数化、写回归测试、分享给团队），就把验证过的脚本作为**文件**收进 `.claude/workflows/`，用 `{ name: 'my-workflow' }` 像调用具名命令一样复用它（这正是第 01 章 1.7 节那句话）。这条路把「脚本即文件」这条朴素事实兑现成一整套工程做法，本章 25.2 往后讲的全是它。

**两条路什么关系？** 它们是同一件事的「轻」与「重」两端。官方台面（按 `s`）让你**零摩擦地把单个 run 变成可复用命令**，适合即兴沉淀；脚本落盘（`.claude/workflows/` 加 `{ name }`）让你**把这些命令当代码来管**，适合系统化经营一个库。新手从路一起步，需要版本、参数、测试、分享时自然过渡到路二。

> 本章接下来主要讲**路二**，因为「怎么组织、怎么命名、怎么参数化、怎么测、怎么分享」这些实质问题，都在脚本落盘这一侧。台面侧的 `s` 存命令已经够简单，《官方操作面板》讲完即可。
...
`description` 会显示在权限弹窗里，用户就靠它来决定要不要授权这次扇出。所以它得回答「这个工作流会干什么、大概多大动静」。对照一下本书里那些真实脚本的 description（都来自实跑 transcript）：
...
**`meta` 必须是纯字面量**（B 节硬约束）。所以 `description`/`whenToUse` 里**不能**用模板插值去拼（比如 `` `Review ${args.target}` ``），因为运行时是在执行之前就静态读取 `meta` 的，那会儿 `args` 还压根不存在。想让描述跟着入参变？把会变的那部分挪进 `log()`（运行时输出），`meta` 保持静态。这是第 26 章会细讲的一条反模式。
...
一个把目标写死的脚本算不上库成员，它就是个一次性脚本。**「一次性」和「可复用」的分水岭就是参数化。** 干这件事的工具是 `args`（B 节：「暴露给脚本的全局 `args`」；第 01 章：「调用方传入的参数对象」）。
...
「脚本即文件」最大的好处，是你的 Workflow 库**整套 Git 工具链直接拿来就用**：diff、blame、PR、tag、回滚，一个不少。比起把提示词散落在对话里，原生 Workflow 在这一点上是代际优势。
...
库要靠得住，就得**能测**。Workflow 能不能测，靠的是第 22 章那条实测结论：**没改过的 `agent()` 在续传时零 token、零工具、几毫秒就返回**（单 agent 的 `hello-workflow`，Run ID `wf_dacbd480-d5d`，首跑 26,338 token、续传 0 token / 8ms，返回值完全一致；完整数据和更大规模的 5-agent 实证见[第 22 章 · 断点续传与缓存](#/zh/p4-22)）。
...
**续传缓存命中的前提是「相同脚本 + 相同 args」**（B 节 + 第 22 章）。所以回归测试里的 `args` 必须跟原始运行**逐字节相同**。这正是 25.4 反复强调「禁用 `Date.now()`，用 `args` 传时间」的回报所在：脚本里要是偷偷用了 `Date.now()`，每次 args 都悄悄不一样，缓存永远命不中，回归测试就退化成「每次全量重跑」，又慢又贵。**可测试性，是纯函数式脚本给你的奖赏。**
...
「脚本即文件」的终极好处：**分享一个 Workflow，就是分享一个 `.js` 文件。** 不用打包，不用运行时安装器。这跟第 23 章那几个系统形成鲜明对比：ccg 要把 hooks 装进 `settings.json` 外加一个 Go 二进制，OMC 要铺一套 `.omc/` 目录结构，OmO 是个 npm 包。原生 Workflow 的分享单元，朴素到就是一个文本文件。
...
**官方台面（按 `s`）也是往这两个层次里存。** 层次一和层次二对应的，正是官方保存对话里给你选的两个落点。操作链路：`/workflows` → 选中那次 run → 按 **`s`** 打开保存对话 → 用 **`Tab`** 在两个位置之间切换（项目级 `.claude/workflows/`（团队共享）⇄ 个人级 `~/.claude/workflows/`），按 **`Enter`** 存下。所以「按 `s` 存命令」和「把 `.js` 收进目录」存的是同一批文件，只是一个走对话、一个直接拷文件。
...
按这份脚手架起步，你的库打从第一天就有了清晰的分组、自解释的索引、参数化契约、测试位、草稿隔离区。往后你每提取一个新模式（第 24 章），它就有机地长大一点，而不是攒成一堆叫 `wf1.js`、`test2.js` 的散件。

---

## 25.9 本章小结

- **建库两条路**。路一·官方台面：满意的 run 按 `s` 存成 `/` 命令、进自动补全、与 `/deep-research` 并列（零文件零配置，多数人从这儿起步）。路二·脚本落盘：把验证过的脚本收进 `.claude/workflows/`、用 `{ name }` 复用（power-user，版本、参数、测试、分享都在这一侧）。新手从路一过渡到路二。
...
- **版本管理**：脚本即文件，所以直接用 Git（diff/blame/tag/PR）。编排被写成代码，「编排变更」头一回成了能 review 的 diff。破坏性变更优先「原地演进加 tag」，被多方依赖了才用版本后缀。
