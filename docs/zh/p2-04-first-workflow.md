...
> 理论讲完了，现在动手。这一章带你从「确认环境」走到「跑通并读懂第一个 Workflow」，把启动、异步、进度、迭代这套循环完整走一遍。每一步都拿**真实运行**的输出来对照。
...
**开启与关闭。** Dynamic workflows 目前是 research preview（研究预览），需要 Claude Code v2.1.154 或更高版本。本书实测用的是 v2.1.156。先 `claude --version` 看一眼，版本低了就先升级。本书的运行跨 v2.1.150 到 v2.1.156，核心机制在 v2.1.156 复核仍然成立。所有付费计划都能用，也支持 Anthropic API 以及 Amazon Bedrock、Google Cloud Vertex AI、Microsoft Foundry。

- **怎么开**：**所有付费计划**（Pro、Max、Team、Enterprise）都能用。**Pro 计划**要在 `/config` 里找到「Dynamic workflows」那一行手动打开。官方文档**没说其余计划（Max/Team/Enterprise）默认是开还是关**，所以别想当然以为它们已经开好了。到你自己的 `/config` 里看同一个开关确认一下。
...
**关于 `CLAUDE_CODE_WORKFLOWS=1`。** 这是个真实存在的环境变量，本书的测试环境里就设着它，但它**不是官方文档给的开启方式**。官方只把 `/config` 当作开启路径，记录在案的环境变量也只有**关闭**用的那一个。所以别把 `CLAUDE_CODE_WORKFLOWS=1` 当成「必须设了才能用」，把它当个底层观测开关看就行。本书 R11 复核会话里用 `printenv` 实测过：它确实在、值就是 `1`，而且 Workflow 工具可用。
...
**两个 0 成本的确认法。** 第一个：直接在对话里说一句「跑个最小 workflow 确认运行时」。句子里带了 `workflow` 这个词，Claude 就会去调 Workflow 工具，开着就能跑，没开它会告诉你用不了。第二个：敲 `/effort`，看滑块里有没有 `ultracode` 这一格，有就说明 workflow 已经「能用」了（道理见 §1.6）。
...
**新手别被「写脚本」三个字劝退：这段脚本不用你手写，是 Claude 替你写的。** 官方给普通用户设计的入口是一条很顺的闭环。你用大白话说一句带 `workflow` 的需求，比如「跑个 workflow 把这仓库的 TODO 扫一遍归类」，Claude 就替你把这段编排脚本写出来。它开跑前会弹一次审批，你过一眼就行，拿不准就点 `View raw script` 看原文。跑完满意的话，按一个键把它**存成一条 `/` 命令**，下次直接复用。所以本章下面那些脚本，你**读懂**就够了，真正动手时是 Claude 写、你审、你存。这条闭环在终端里怎么按键操作（审批的 4 个选项是什么、按 `s` 怎么存命令）在[《官方操作面板》](#/zh/p2-ops)有照着就能上手的完整演示；本章专注「脚本本身长什么样、怎么读懂它、怎么迭代它」。
...
脚本一交给 Workflow 工具，它**不会等跑完**，当场就甩回来一张回执。这是真实输出：
...
这张回执对应的就是 `_grounding.md` B 节里 `WorkflowOutput` 的那几个真实字段。一一对上，列成一张表：
...
**回执的 `status` 只有两种取值。** 按 `_grounding.md` B 节，`WorkflowOutput.status` 就是 `"async_launched" | "remote_launched"`，没有第三种，也**没有**那种表示「已完成」的同步状态。本地跑就是 `async_launched`（你这次就是），跑在 CCR 远端就是 `remote_launched`（这时没有 `runId`，续传句柄改用返回的 session URL）。语法检查没过的话，返回会多带一个 `error` 字段（见 4.7）。记牢这一条，你就不会再指望「调一下 Workflow 就能直接拿到结果」。

</div>

<div class="callout info">

**为什么要做成异步的？** 因为一个工作流可能扇出几十个 subagent，跑上几分钟甚至更久。做成异步，你启动完就能接着干别的，跑完了再收到通知。所以记住一句：**Workflow 工具返回的是一张「已启动」回执，不是结果**。真正的结果在完成通知里。
...
- 最简单的一次 agent 往返约 **5.5 秒 / 2.6 万 token**。拿它当基线单位，就能估更大的工作流要花多少。
...
脚本已经落了盘（就是回执里的 `Script file` / `WorkflowOutput.scriptPath`），所以迭代一个工作流不用每次都把整段代码重发一遍。这就给了你一个**「改盘上文件 → `scriptPath` 重跑」的迭代闭环**：
...
要提醒一句：`resumeFromRunId` **只在同一个 session 里有效**。你要是退出了 Claude Code，那份缓存就跟着 session 一起没了；下次再进来，这个工作流会**从头跑**，不会接上之前的进度。所以续传是「同会话内」的能力，别指望它跨重启。续传的细节见第 22 章。
...
第一次写 Workflow，下面这几个坑几乎人人都要撞一遍。一个个拆开看，「错」长什么样、「对」该怎么写。

**① `meta` 不是纯字面量（包括「在 `meta` 里算值」）。** `meta` 必须是「死」字面量，运行时在**静态解析阶段**就把它读了，任何变量引用、函数调用、展开、模板插值都会让它拒绝启动。新手尤其爱在 `meta` 里「顺手算一下」，比如拼个名字、按日期生成描述，这恰恰是被坑的重灾区：
...
**② schema 漏了 `required` 字段。** 传 `schema` 的时候，别写完 `properties` 就收手，还得在 `required` 里把**必须出现**的字段列上。不然模型可能合法地把它漏掉，你下游 `r.sum + 1` 就会拿到 `undefined`：
...
**③ 把它当同步调用，以为「调完就能拿到结果」。** 这是最伤人的一个认知错。Workflow **始终异步**：调用立即甩回一张回执（`status` 只会是 `async_launched`/`remote_launched`，见 4.3），结果在**完成通知**里。任何「`const result = Workflow(...)` 然后立刻用 `result.sum`」的写法都是错的，因为那一刻 `result` 只是回执，不是产物。

**④ 语法错误。** 脚本语法检查没过，`WorkflowOutput` 会带一个 `error` 字段告诉你错在哪，工作流**不会启动**。先在本地把脚本写对，再交上去。

<div class="callout warn">

**别在脚本里用 `Date.now()` / `Math.random()` / 无参 `new Date()`。** 它们会破坏可重放性，让续传缓存失效（见 4.5）。这道禁令实测是**两层**拦你的（提交期源码扫描 + 运行时陷阱），连写在注释或字符串里都拦，`try/catch` 也接不住。两层的完整机理、报错原文、以及绕过/接住的误区，集中在 [App B 的 B.5 / B.19](#/zh/app-b)，因果链见 [第 22 章](#/zh/p4-22)。这里你只要记住替代写法：要时间戳就用 `args` 传进来，要随机性就拿 agent 的下标 `index` 去改提示词。
...
- 先确认工作流在你会话里可用：**所有付费计划**都能用，Pro 要在 `/config` 的 "Dynamic workflows" 行手动打开。官方文档没说其余计划默认是开还是关，所以到你自己的 `/config` 里看一眼那个开关（关闭三法见 §4.1）。`CLAUDE_CODE_WORKFLOWS=1` 只是个底层观测开关，不是官方开启路径（两层见 [§1.5](#/zh/p1-01)）。拿不准就让 Claude 跑个最小工作流确认一下。
...
- 官方闭环的**收尾**：跑通一个满意的 run，在 `/workflows` 视图里按 `s` 就能把它**存成一条 `/` 命令**，下次直接复用。对新手来说，「构建你自己的 workflow」最自然的入口就是这条（怎么积累成一个库见 [第 25 章](#/zh/p5-25)）。
- 新手四坑：① 在 `meta` 里算值（必须纯字面量）；② schema 漏了 `required`；③ 把它当同步调用、以为立刻就能拿到结果；④ 语法错误，进 `error` 字段、不启动。

<div class="callout tip">

**搭你自己的 workflow · 6 步。** 这条主线散落在全书各处，这里给你一张可见的目录，照着走就行：
