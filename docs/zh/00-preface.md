...
两边 subagent 烧掉的 token 差不多，约 70 万和 77 万，干的是同样多的活。差别在回到你眼前的量：用 workflow 是 **357** 字符，不用是 **47,080** 字符，**差 132 倍**。这 47,080 字符进了主对话就留在那儿，之后你每问一句、每改一行，模型都得带着它一起算，越往后越慢、越贵。`pipeline` 把原始评审留在沙箱里，只把结论递给你。

时间上两边没拉开，都在 4 分钟上下，这次手动那边还略快一点。同样多的活，耗时本来就接近。这次墙钟不算 workflow 的便宜；`pipeline` 省墙钟是另一回事，要在更长的流水线上才显出来，[第 8 章](#/zh/p2-08)用屏障的实测单独算这笔账。

所以 workflow 帮你守住两样东西：主对话里的空间，和一条不漏的把握。后面的章节，教你怎么把这种脚本写得稳、又能反复用。

> **这两次运行**：用 workflow 臂 `wf_6fc26e37-02d`（`pipeline`，12 个 agent，回灌 357 字符）；不用臂 `wf_372d53bf-419`（用 `parallel` 两道屏障模拟手动「收齐再开下一批」，12 个 agent，prose 回灌 47,080 字符）。要说明的是，「不用」臂是对真人手动的保守估计：它没算上你逐条读结果、决定下一步的时间，真按手动来只会更慢、回灌更多。两次都在本机 v2.1.156 实测，原始数据见 [`assets/transcripts/examples-r14.md`](https://github.com/AGI-is-going-to-arrive/workflow-cookbook/blob/main/assets/transcripts/examples-r14.md)。
