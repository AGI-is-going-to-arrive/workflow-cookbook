...
重点在第一行：同样一摊活，回到你眼前的东西差了 **132 倍**。那四万多字符会一直跟着你，之后每问一句、每改一行，模型都得连它一起再读一遍，越往后越慢、越费钱。用 workflow，原始评审留在沙箱，只有结论回到你眼前。

速度反倒没差，两边都是 4 分钟上下，所以别冲着「更快」去用它，省时间得靠更长的多阶段流水线（见[第 8 章](#/zh/p2-08)）。后面的章节，就教你把这种脚本写稳、还能反复用。
