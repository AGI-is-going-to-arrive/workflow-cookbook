...
两边干的活其实一样多，subagent 烧的 token 都差不多，约 70 万和 77 万。不一样的是回到你眼前的量。用 workflow，你只拿到 **357** 字符的结论；不用，**47,080** 字符的评审全文全堆到你面前，**差 132 倍**。这四万多字符一旦进了对话就赖着不走，之后你每问一句、每改一行，模型都得连它一起再读一遍，越往后越慢、越费钱。用 workflow，这些原始评审留在沙箱里，只有结论递到你手里。

速度上两边没差，都是 4 分钟上下，这次反倒是手动那遍快了一丁点。活一样多，花的时间本来就接近，所以这次别指望 workflow 帮你跑得更快。真要靠它省时间，得换成更长的多阶段流水线，[第 8 章](#/zh/p2-08)拿真实数据单算那种情况。

所以 workflow 帮你的是两件事：把一堆中间过程挡在对话外面，再保证每条活都做到、不漏掉。后面的章节，教你怎么把这种脚本写得稳、又能反复用。

> **这两次运行**：用 workflow 那遍是 `wf_6fc26e37-02d`（`pipeline`，12 个 agent，最后只回来 357 字符）；不用那遍是 `wf_372d53bf-419`（用 `parallel` 两道屏障模拟手动「收齐一批再开下一批」，12 个 agent，回来的是 47,080 字符的 prose）。多说一句：「不用」那遍已经是按手动最快的情况算的，没算上你自己逐条读、决定下一步要花的工夫，真用手动只会更慢、堆回来的更多。两次都在本机 v2.1.156 上跑的，原始数据见 [`assets/transcripts/examples-r14.md`](https://github.com/AGI-is-going-to-arrive/workflow-cookbook/blob/main/assets/transcripts/examples-r14.md)。
