...
>
> 本章基于一次真实运行（Run `wf_53da9a06-915`，11 个 agent / 311,134 token / 61,660ms，5/5 确认），它顺带演示了对抗验证最惊艳的一面：**验证者反过来纠正了猎手。** 在那之上，我们把一个**生产级 bug-hunter 工作流**从头搭出来（**finder 池 → 对抗证伪 → 综合**），再看它怎么靠 **pigeonhole 提前退出** 和 **连续 K 轮 dry-streak** 既省钱又防漏。
>
> <div class="callout warn">
>
> **关于「自带 bughunt」的一条版本说明（必读）。** 早期 v2.1.150 的内置注册表里确实有 `bughunt` / `bughunt-lite` 两个具名工作流（Run `wf_2b04881f-6a9` 实测列出过），本书前几版也以「直接调内置 `bughunt`」为前提。但**到 v2.1.156，内置具名工作流注册表已经只剩 `deep-research` 一个**（Run `wf_03e38250-1bb` 报错原文 `Available: deep-research`），与官方文档「Claude Code 只自带 `/deep-research` 这一个 bundled workflow」完全一致。`bughunt` / `bughunt-lite` **已不在注册表、不可再依赖**（详见 [附录 A.13.1](#/zh/app-a)）。所以本章不再教你调内置，而是带你**自己写一个 bug-hunter 工作流**，这也正好落在全书「构建你自己的 workflow」这条主线上。下文凡提到 `bughunt` / `bughunt-lite`，都请当成**早期内置的设计参照**来读：它们当年的编排骨架仍有借鉴价值，但已经不是你今天能直接调起来的命令。
>
> </div>
...
1. **假阳性（false positive）**：模型特别爱报告点什么。你说「找 bug」，它就算一个真 bug 都没找到，也会编出几个看着挺像的「bug」来交差，因为对它来说，交白卷就等于没干活。
2. **错误论证（wrong argumentation）**：就算 bug 是真的，模型给的「为什么错」也可能是错的。它可能症状抓对了，机理却说岔了。

两个坑叠在一起，结果就是：**光看一个猎手 agent 的输出，你既不知道它找全了没，也不知道它找对了没。** 本章的配方就用三段编排，把这两个不确定性都摁下去：
...
- **② Verify（证伪）** 管「找对」：对**每一条**疑似 bug，派 N 个**互相独立**的验证者，并明确要求它们默认证伪。这样举证责任就压到了「这是真 bug」这一方头上。这是第 17 章对抗验证的直接应用。
- **③ Synthesize（综合）** 收口：用**代码**计票、去重、排序，产出最终的确认清单。

<div class="callout info">

**为什么是「证伪」而不是「确认」？** 因为确认偏误（confirmation bias）是单向的：你问一个 agent「这是不是 bug」，它倾向于点头。命令它「尽力**证伪**这条、拿不准就判 refuted」，它才会主动去找反例。把默认值设成 refuted，**模型的沉默和犹豫就都倒向了「不算数」**，假阳性自然被滤掉。这个「默认证伪、翻转举证责任」的机理由第 17 章（[对抗验证](#/zh/p4-17)）讲透，本章是它在「猎 bug」场景下的落地。
...
**这个最小形态足以建立直觉，但它有意省略了三件「生产级」的事**，本章后面逐一补上：① Hunt 只有一个 finder，规模一大就会漏，对应 15.5 的 finder 池；② 每条 bug 跑满全部证伪者才计票，多数已否决也不早停，对应 15.6 的 pigeonhole；③ 单轮 Hunt，漏掉的尾部 bug 永远找不回，对应 15.7 的 loop-until-dry。先吃透最小形态，再加这三层。
...
11 个 agent 的账这么算：`1 个 finder + 5 条 bug × 2 个证伪者 = 11`。token 约 31 万，墙钟约 62 秒。注意墙钟远小于 11 个 agent 串行跑的总和，因为 5 条 bug 的证伪在 pipeline 里是**重叠**进行的：墙钟取决于关键路径，而不是所有 agent 耗时之和（第 8 章）。

<div class="callout info">

**为什么拿一个合成样本当猎手对象？** 因为想验证猎手到底准不准，你手里得先有**已知的标准答案**。`buggy-cart.js` 里每个 bug 都埋了种子注释，所以「找到 5/5」是个能逐条核对的硬指标，而不是「看着找了不少」的模糊感觉。真实项目里没有这种标注，这恰恰是 ② Verify 存在的理由：用对抗证伪去**逼近**真值。
...
**这正是对抗验证值钱的地方**：它不只过滤假阳性，还能**顺手修正真阳性里的错误推理**。一个只会点头附和的检查者永远发现不了这种问题；只有被要求「默认证伪、拿不准就判 refuted」的验证者才会真去较真，连藏在前提里的小瑕疵都不放过。证伪者交回来的不只是一张「真/假」选票，还附带**一份可审计的推理**，而这份推理本身就能纠正上游。这也是为什么 15.2 的证伪 schema 里 `reason` 是必填字段。
...
早期 v2.1.150 的内置注册表里有两个跟 bug 猎手同源的具名工作流 `bughunt` 与 `bughunt-lite`（Run `wf_2b04881f-6a9` 实测：调用未知具名工作流时报错并列出当时的清单 `bughunt, bughunt-lite, deep-research, plan-hunter, review-branch`）。**注意：到 v2.1.156 这俩已不在注册表了**（只剩 `deep-research`，见本章开头的版本说明与 [附录 A.13.1](#/zh/app-a)）。所以下面不是教你去调它们，而是**拿它们当年的编排骨架当蓝本，自己把同样的结构写出来**。下表把这套骨架拆成「固定池」与「自我复制池」两种形态：
...
- **rapid finder**：快、浅、广撒网，先把一眼能看出的可疑点捞上来（可以用 `model:'haiku'` 降成本）。
...
**finder 池一定要在「汇流去重」后再进证伪，且去重用代码而非 agent。** 多个 finder（尤其 rapid 与 deep）必然报出重复的 bug；若不先去重，同一个 bug 会被 N 组证伪者各验一遍，token 白白翻几倍。去重是**确定性操作**（同输入同输出），用 `Set` + 归一化键零成本就能完成，这与第 18 章「凡确定性操作交给代码、判断才交给 agent」一脉相承。让 agent 帮你去重，既贵又引入不确定性。
...
这就是 **pigeonhole（鸽笼）提前退出**：把「多数」看成一个可以提前够到的阈值，一旦某一方的票数已经锁死胜负，就在**逻辑上**提前判决，不再傻等剩下的票。但有个前提要记牢：**已经发出去的 agent 通常还是会跑完，只是结果被忽略**（机制见本节下方的提示框、`_grounding.md` 与第 18 章）。想在**物理上**真的少发 agent，得**分批投票**，先发够到多数线的票，平票或者接近时才追加后面的票。
...
能省多少？在「5 票多数」下，最快第 3 票就能定局，省下 2 个证伪者，**接近 40% 的验证成本**；而且越是一边倒的 bug（真 bug 全 confirm、假阳性全 refute），省得越多。

实现上有个坎：`parallel` 是**屏障**（要等齐所有 thunk），天生不支持中途叫停。想做 pigeonhole 提前退出，得换成一种「可以提前结算的竞速」结构。下面是一个**示意**骨架，用 `Promise` 竞速加一个计票闭包实现。注意它已经超出了 `parallel` 的标准用法，只是拿来演示思路：
...
**「提前退出」省的是「等待和决策」，不一定省「已经在途的 agent」。** 据 `_grounding.md`，`parallel` 一旦把 N 个 thunk 都启动，它们就并发跑起来了。上面的竞速骨架能让你**在多数锁定的那一刻立刻拿到结论、不被阻塞**，但已经发出去的 agent 调用通常照样跑完，只是结果被忽略。想真正在物理上少发 agent，得**分批投票**：先发 3 票（多数线），只有平票或接近时才追加第 4、5 票。这一步才把 pigeonhole 从「逻辑早停」升级成「物理省钱」。但不管哪种，核心都一样：**别为一个已成定局的判决付满额成本。**
...
**dry-streak 能防漏尾，但硬上限绝对不能去掉。** `while` 条件里的 `round < MAX_ROUNDS` 和 `budget.remaining()` 检查是**安全带**，不是摆设。万一碰上一个总能编出新疑似的猎手，dry-streak 会被反复清零、循环永远退不出去。据 `_grounding.md`，`budget` 是硬上限（达到 `total` 后再调 `agent()` 就抛错），单工作流生命周期 agent 总数上限 1000 是最后那道全局安全网，但你**绝不该**指望它们来终止业务循环。正确的纪律是：**dry-streak 决定「正常情况下何时停」，轮次上限 + budget 决定「最坏情况下何时强停」，三者缺一不可**（详见第 18 章 §18.3）。
...
**① 验证者必须各自独立。** 用 `parallel`（或竞速）让多个证伪者**各判各的**、互相看不见，这样它们犯的错才不相关，多数表决也才有意义。一旦让它们看到彼此的票，就退化成随大流，投票也就白投了。
...
**⑤ finder 池规模要配得上目标规模。** 目标小（单文件）一个 finder 就够；整条分支用固定池（`bughunt-lite` 那种 3+2 形态，5 个 finder）；只有当规模完全摸不清、漏报代价又高时，才上自我复制池 + dry-streak（`bughunt` 那种形态）。
...
Bug 猎手很容易和第 10/11 章的 review、第 17 章的对抗验证搞混。它们用的是同一批底层 primitive（`agent`/`pipeline`/`parallel`/`schema`），但**奔着的目标**不一样。把边界划清楚，你才知道该选哪个配方：
...
- Bug 猎手 = **Hunt（finder 池找疑似）→ Verify（每条 bug 派 N 个独立、默认证伪的验证者 + 计票）→ Synthesize（代码去重排序收口）**。它专治「未知规模 + 不可信」的双重难题。设计上的五条要点见 15.8 节。
- 真实运行 `wf_53da9a06-915`（11 agent / 311,134 token / 61,660ms）：5 个种子 bug 全部找到并 2:0 确认；验证者还**纠正了猎手的错误论证**（字符串拼接那条）。
- 三层生产级加固：**finder 池**（固定池 `bughunt-lite` 的 3 rapid + 2 deep，或自我复制池 `bughunt` 的 deep-until-dry-streak，rapid 撒网、deep 深挖，finder **流入**无屏障的证伪管道）；**pigeonhole 提前退出**（多数票锁定胜负就别再投，逻辑早停省「等待」、分批投票省「物理 agent」）；**loop-until-dry + dry-streak**（用「连续 K 轮 K≥2 无新增」而非「一轮没找到就停」来防漏尾，硬上限 + budget 是不可去掉的安全带）。
