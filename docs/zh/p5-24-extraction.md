...
> 上一章逐一拆解了四个社区系统，反复标出「这一点值得借鉴」。但借鉴不等于把别人的提示词复制粘贴过来。那样拿到的只是一个长在别人宿主上的器官，移植到原生 Workflow 里会立刻排异。本章提供一套可反复使用的方法，将别人系统里的好想法一步步拆开、剥离，重写成你自己的、确定的、能复用的 Workflow。
>
> 这套方法有四个动作，一个都不能少：**解构**（看清它到底怎么运转）、**抽象**（剥掉宿主，把模式的本质提出来）、**适配**（用 `phase`/`schema`/`parallel`/`pipeline` 重写）、**验证**（真跑一遍，和原系统比一比）。下面就拿第 23 章那四个真实案例，把这四步从头走一遍。

---

## 24.1 为什么需要「术」，而不是直接抄

先看一个注定失败的场景，就能理解「术」要解决什么问题。

假设你读了 superpowers 的 `subagent-driven-development/SKILL.md`，一眼相中它的「两段式评审」：每个任务先过一道 spec 合规评审、再过一道 code quality 评审，各自循环到通过为止。你很自然地想：**把这两段评审的提示词拷到我项目里就行了吧？**

于是你把那两段 markdown 拷进自己的 `.claude/skills/`，以为「两段式评审」到手了。但很快会遇到三个问题：

1. **它依赖一个你没有的宿主。** superpowers 的评审能「循环到通过」，靠的不是那两段提示词本身，而是它整套 `SessionStart` 注入的「行为宪法」加上 skill 链和 checkbox 状态文件。提示词单独拎出来，就成了第 23 章描述的那种「死代码」：没有 bootstrap，skill 根本不会被强制触发。
2. **它的「保证」是概率性的。** 即使宿主齐全，superpowers 的评审循环本质上也是「用提示词请求模型再评一遍」。模型可能真去评，也可能判断「差不多了」就跳过。这是一道软约定，不是硬控制流。
3. **它产出的是给人看的文本，不是给程序消费的数据。** 评审结论写在对话里，无法用代码判断「这一轮到底过没过、要不要再来一轮」。

<div class="callout warn">

**直接抄的病根在这。** 你抄来的是**实现**：某个宿主上「提示词 + 钩子 + 状态文件」的特定组合。可你真正想要的是**模式**：「写完先查规范、再查质量，不过就重来」这个控制结构。实现长在宿主上，能搬走的只有模式。精华提取术的全部要义，就是把模式从实现里剥出来，再拿原生 Workflow 的骨架重新长出一个实现。

</div>

回到第 23 章那个贯穿全书的洞察：这四个系统都生在原生 Workflow 之前，它们靠「提示词 + Hook + 状态文件」硬凑出了一个确定性编排引擎。它们发明的那些精华，包括验证门、持久循环、磁盘状态、越界护栏，全是**模式**。而它们装这些模式的方式（软约定、钩子注入、文件续命）只是**那个年代的实现**。

原生 Workflow 给了你一套更趁手的承载工具：
- `pipeline` / `parallel` / `phase`：用**代码**写控制流，保证够硬。
- JSON Schema：把「产物长什么样、合不合格」从一段自由文本变成**机器能判定的契约**。
- `agent({ schema })` 的工具层校验加自动重试：把「请求模型再试一次」从不确定的期望变成**运行时纪律**。

所以提取术的产出物，就是一个把别人的模式焊到原生骨架上的 Workflow 脚本。下面这张图是全章总纲：
...
「解构」要回答一个问题：**这个好想法，真正起作用的机制到底是什么？** 注意「真正」二字。人们对自己系统的**说法**，常常和它**实际的实现**对不上。提取的第一性原理只有一句：**只信源码，不信营销词。**
...
这一步需要**翻源码**，找到真正把意图落地的代码/配置。第 23 章已经完成了这项工作，结论直接引用（信源是对各仓库源码的真实阅读）：
...
注意每一行「实际」列都精确到了**文件名、钩子名、字段名**。这是解构的及格线：**能说清它在哪个文件、用什么数据结构、在哪个生命周期点起作用。** 达不到这个粒度，解构就还没完成。
...
|---|---|---|
| 「跑完不一定算完，要看是否满足判据」 | **本质** | 这是「完成判据循环」的核心思想，与任何宿主无关 |
...
剥离后结论很清楚，本质就一句话：**能不能结束，应由一个可编程的判据决定。** 其余部分都是 OMC 在「没有原生循环」这一限制下不得不搭建的脚手架。
...
>
> 转译：两道评审是**顺序**关系（先 spec 后 quality），对应 `pipeline` 的两个 stage。每道评审的「过/不过」得能驱动判断，所以各配一个带 `pass: boolean` 门控字段的 `schema`。「不过就重来」就是在 stage 内部对单份产物跑有界 `while`。

**OMC Stop 钩子** 抽象为：
> 「一直往前推，直到一个可编程的判据点头『可以收工了』。」
>
> 转译：「反复直到判据」对应 `while` 加一个 `done`/`accepted` 门控字段（与第 18 章「循环到干」同构）。「可编程判据」就是一个独立的验收 `agent({ schema })`，schema 里带个 `accepted: boolean`。

**ccg 磁盘面包屑** 抽象为：
> 「让后面的步骤拿得到前面步骤的**结构化产物**，这样它随时都清楚『当前进展和已知事实』，不用去翻会被压缩的对话历史。」
>
> 转译：ccg 之所以用「磁盘 + 每轮注入」，是因为它的步骤散在好几个对话回合里，会被上下文压缩冲掉。而原生 Workflow 的脚本是**一个连续的 JS 闭包**：前一个 `agent()` 的返回值，直接就是后一个 `agent()` 提示词里的变量。`task.json` 的本质是「把状态外化、显式传下去」，这在 Workflow 里**退化成了普通的变量传递加 structured output**，磁盘根本用不上。

**OmO 工具层护栏 + Category 委派** 抽象成两句：
> 「①规划者的产物只能是『计划』，不能夹带『对代码的副作用』；②『用什么模型』该由任务的_语义类别_说了算，别让一堆硬编码模型名散在提示词里。」
>
> 转译：①「产物只能是某种形状」对应 `agent({ schema })`，再用 `additionalProperties: false` 把 diff/补丁这类字段**从结构上**挡在门外，然后把「执行」拆成一个独立 phase（角色分离）。②「按类别选模型」对应一张 `MODEL_BY_CATEGORY` 查找表加 `opts.model`，让 `category` 字段去驱动派发。
...
这是把「方法论纪律」焊成「确定性质量闸」的演示。模式抽象已经给好了：两段评审是顺序关系，用 `pipeline` 的两个 stage，每段各带一个 `pass` 门控，不过就有界重来。
...
这段脚本把 superpowers 那套「靠提示词请求 re-review」的软约定，变成了**两道确定性闸门**：第一道 `SPEC_SCHEMA.pass` 不为 true，第二道不会开启（quality agent 不派发，节省 token）；两道都过，`accepted` 才为 true。`pipeline` 让每个任务**各自独立**流过两道闸，10 个任务的wall-clock约等于最慢那个任务走完两道闸的时间，而不是「全部 spec 评审完、再全部 quality 评审」（那是 `parallel` 屏障的低效写法，第 26 章会专门分析）。

那「不过就重来」呢？上面这版只是「评一次、给个结论」。要实现 superpowers 真正的「循环到过」，需要在闸内部加**有界重试**。重试意味着「评审 → 不过就修 → 再评」，这实际上退化成了第 12 章的 GCF（生成-批评-修复）循环。完整写法如下：
...
这里每个细节都延续了前面章节确立的纪律：`maxRounds` 对应第 18 章反复强调的「刹车是纪律，不是选配」；`pass` 门控字段与第 18 章「`done: boolean` 门控」同构；独立的 reviewer 和 fixer 来自第 12 章的原则「批评必须交给独立 agent，否则它会替自己辩护」。**这就是提取术的复利**：为一个案例积累的纪律，能原样搬到下一个案例。

### 案例二：OMC Stop 钩子完成判据 → `while(!done)` 循环 + 验收 schema

OMC 的精华抽象为「一直往前推，直到可编程判据确认可以收工」。这个模式在第 18 章已经有了完整的 Workflow 化身（「循环到干」），这里换一个更贴近 OMC「PRD 每条 story 都 `passes:true` 才算完」的形态来演示，即**逐条验收，全过才停**：
...
| 同一 session 内停了再续 | `resumeFromRunId` 续传（仅限同一 session；进程级 crash/退出后下次 session 从头跑，第 22 章） |

OMC 为「让循环可编程」搭建的全套脚手架（钩子、状态文件、回注文本），在原生 Workflow 里**坍缩成了一个 `while` 加几个局部变量**。这并非 OMC 设计不当，而是它诞生在没有原生循环的年代。这也正好将第 23 章的结论具体兑现了：**原生 Workflow 提供了它们缺少的那副确定性骨架。**

<div class="callout warn">

**不要把 OMC 的精华移植成「无界循环」。** 移植「反复往前推、直到判据确认」这类持久循环时，**必须**避开无界循环：Workflow 版一定要带上 `MAX_ROUNDS` 加 `budget.remaining()` 兜底，这是第 18 章的铁律（模型给的 `done`/`accepted` 是概率性判断，可能迟迟不放行）。返回值中应标注 `hitCeiling`，让调用方知道本次是「真验收通过」还是「触及上限被迫收工」。不能让「boulder never stops」变成「token never stops」。

</div>

### 案例三：ccg 磁盘面包屑 → structured output 产物传递

这个案例的「适配」最特殊，因为在抽象阶段我们就发现了：**ccg 的磁盘面包屑在原生 Workflow 里基本是白送的。** 但「白送」不等于「没活干」，它在 Workflow 里对应一个**正面实践**：用结构化产物在阶段之间显式传「已知事实 + 当前进展」，省得后面的 agent 去猜，或去读那段会被压缩的历史。

ccg 的 `task.json` 加 `<ccg-state>` 面包屑，本质上是在回答「下一步该知道什么」。在 Workflow 里，这由**前一阶段的 structured output 直接传入后一阶段的提示词**即可实现：
...
**ccg 这一课真正有价值的地方，是「显式传、结构化地传」这个原则，而不是「把磁盘搬进内存」。** 不少人写 Workflow 时图省事，让后一个 agent「自己去读文件 / 自己再勘察一遍」，这既慢，又可能读出不一致的事实。ccg 用磁盘面包屑强制「状态显式化」，这个**习惯**值得保留：让每一阶段的关键产出都走 `schema`，再显式传给下一阶段。原生 Workflow 把这件事的成本压到「一个变量加一次 `JSON.stringify`」，没有理由不做。
...
>
> **本质二（Category 委派）**：OmO 不按模型名派活，而是按**语义意图**（`category`）派。LLM 只声明「这是一件什么类别的事」，运行时再映射到具体模型。一句话概括：把「用什么模型做」变成一张能热插拔的映射表，别让它在提示词里硬编码。

**适配本质一：用 `schema` 加角色分离，复刻「规划者只产出计划」。** 先明确一个边界：原生 Workflow 的**脚本**（meta 之后的代码）没有 FS/Node API，但**叶子 agent 有完整 IO**（Read/Write/Bash），所以 agent **能够**写宿主文件，OmO 那道「工具层 throw」拦截的正是这种副作用。`schema` 无法复刻这道拦截：它只在**工具调用层约束规划者的返回结构**，`additionalProperties: false` 让规划者**在结构上**无法夹带 diff/补丁字段，但它**无法阻止规划者另外调用 Write/Bash**。因此「规划者物理上写不了码」这条，单靠 schema **无法实现**。真正要封堵，需要**限制规划者 agent 的工具/权限**（比如换一个不带 Write/Bash 的 `agentType`），或者**在规划阶段不执行任何副作用动作**，只把产物当数据传给下游。schema 在这里的真正作用是后者的配套：保证传下去的是一份**干净的计划**，再让一个**独立的执行者阶段**去消费它。
...
两边给出的**保证是等价的**：规划者都没法把「对代码的副作用」夹带进自己的产出。OmO 靠拦工具调用，原生 Workflow 靠约束产物形状加阶段角色分离。后者甚至更干净，因为它干脆不给规划者「写文件」这个能力，而不是等它写了再拦。
...
这张映射表的价值与 OmO 的设计动机一致：**把「用什么模型」从提示词里提取出来，集中到一处配置。** 模型换代时（比如 haiku 升级、新增一档中端模型），只需改 `MODEL_BY_CATEGORY` 一行，所有按类别派发的步骤自动跟着走。提示词里不出现模型名，也就不会产生「模型名散布各处、一改就漏」的维护债务。

<div class="callout warn">

**把 schema 的边界说清楚（呼应 24.5 的立场）：`schema` 锁住的是产物的_形状_，不是产物的_真实性_。** 上面的 `planner-executor` 能保证「规划者交出来的是一个纯计划对象」，但**不能**保证「这个计划是对的」，更不能保证「执行者真照做了」。OmO 除了工具层护栏，还有一层 **system-reminder 注入**（`VERIFICATION_REMINDER`：「子 agent 说完成了，它在撒谎，去验证」），而**这一层在原生 Workflow 里没有精确对应物**：Workflow 没有「每轮往子 agent 上下文里硬塞一段提醒」的钩子。因此 OmO「不可信验证」这条精华在原生侧的等价物**是一个显式的验证阶段，而不是 schema**：需要自行添加一个独立的 verify-agent（像案例一的 quality 闸、案例二的 accept 闸），用它的 `pass`/`accepted` 门控去判断执行者的产物。**不要指望 `schema` 代替验证；它只管形状，验证需要独立的人或 agent。**
...
适配产出了脚本，但**没跑过的脚本不算提取成功**。验证回答两个问题：①它能跑通吗？②它确实复刻了原系统的精华吗（还是只是看上去像）？

### 验证的三个层次

**层次一：能跑（语法 + 运行）。** 把脚本交给 Workflow 工具。回顾第 01 章：返回是异步的，立刻给出 `taskId`/`runId`；如果 `meta` 不是静态字面量（static literal）、或脚本有语法问题，`WorkflowOutput` 会带上 `error`（语法检查未通过）。这一层验证的是「骨架是否立得住」。

**层次二：行为对（产物符合预期）。** 跑完后查看完成通知里的返回值。这里 `schema` 相当于免费的断言：只要 `agent({ schema })` 返回了，就说明产物**已经**通过了结构校验。但结构正确不等于语义正确，还需要人工核查：两段评审真的拦住了不合规的 diff 吗？验收循环真的是全部通过才停的吗？
...
本书第 12 章的 GCF 配方，是一次**真跑过**的「生成 → 对抗式批评 → 修复」，正好是 superpowers 两段式评审的「单段实跑版」。以它来演示「等价验证」的具体形态，因为它有真实数据可以锚定：
...
验证还有一个常被忽略的用途：**用真实运行数据校准对成本和收敛的直觉**，避免移植来的模式在实际规模上失控。本书的真实运行提供了几个可直接引用的锚点。
...
| frontend-review（3 维并发评审 + 综合） | `wf_4c5caabb-b73` | `agent_count=4`、`total_tokens=221648`，印证「token≈agent 数×每 agent 上下文」，移植「多角度评审」前可据此估成本 |
| nested-parent（嵌套子流程） | `wf_85e22b38-126` | 子流程的 agent **计入父流程**的 `agent_count`/`budget.spent()`，移植「子流程委派」模式时，预算要按父子合计算 |

<div class="callout info">

**judge-panel 那次运行还有一个值得记录的观察**：3 名评委在打分理由里写明，它们**确实去读了 `docs/en/p2-08` 和 `assets/_grounding.md`，交叉核对**了书里几个数字（8.4s/78844 token、min(16, cores−2)、1000 上限……），结论是「zero factual errors」。注意这是**这一次运行**中、被配置了打分 rubric/schema 的评委表现出来的行为，**不是** Workflow 的普遍保证：`agent({ schema })` 校验的是产物结构，并不强制 agent 去外部求证。应将其视为一条经验性观察：给评委一份明确的 rubric 加 schema，有助于（而非保证）引出「主动核对事实」这类行为。如果需要的是**强制**的验证门（像 OmO 的 VERIFICATION_REMINDER 那种「子 agent 说完成了就去验证」），仍然需要显式的提示词指令或额外的验证 agent，schema 本身无法提供这种保证。
...
为了让这套方法可操作，把四步固化成一张**工作单**。下次在任何系统里发现一个好想法，照着填写即可。
...
flowchart TD
  ...
  D -->|"通过"| E["存为具名 Workflow<br/>（见第 25 章）"]
  D -->|"不通过"| A
```
...
走完这张工作单，得到的不再是「抄来的提示词」，而是一个**完全理解、能确定性复现、能纳入自己库中**的 Workflow。下一章讲述如何把这些产出物收进一个易维护、可分享的个人 Workflow 库。

---

## 24.7 早期内置 workflow：五个免剥离的现成范本

本章前面教的提取术，最费力的一步始终是**剥离宿主**：四个社区系统的精华都长在「提示词 + Hook + 状态文件」这层宿主上，必须先从宿主上剥离，才能焊到原生骨架上。但有一类范本天生就省掉了这一步。早期 v2.1.150 的内置注册表里**自带过五个具名 workflow**，它们**本身就是原生 Workflow**，根本没有宿主可剥。所以它们是最干净的偷师对象：你能直接看清「独立多视角 → 对抗验证 → 综合收口」这套质量套路的骨架，不用先做一遍剥离手术。这一节把这五个范本逐一拆开，正好从横向印证本章的方法论：能搬走的只有模式，而内置 workflow 把模式以最纯的形态摆在了你眼前。

<div class="callout warn">

**先交代一条版本事实，否则下文会误导你。** 这「五个内置」是**早期 v2.1.150** 注册表里的真值（Run `wf_2b04881f-6a9` / `wf_28a5d455-300` 都列出过五件套）。但**到 v2.1.156，内置具名工作流注册表已经只剩 `deep-research` 一个**（Run `wf_03e38250-1bb` 报错原文 `Available: deep-research`），与官方文档「Claude Code 只自带 `/deep-research` 这一个 bundled workflow」完全一致。也就是说，本节里**唯一今天还能直接调起的内置只有 `deep-research`**；`bughunt` / `bughunt-lite` / `plan-hunter` / `review-branch` 这四个**已不在注册表、不可再依赖**（详见 [附录 A.13.1](#/zh/app-a)）。那为什么还留着拆这五个？因为本节的价值从来不是「教你调内置」，而是「教你从一个干净的原生 workflow 里**反推出可复用模式**」：这五个当年的注册简介，恰好把五种最典型的质量骨架摆在明面上，是绝佳的**自建蓝本**。下文凡提到这四个已下架的名字，都当成**早期内置的设计参照**读，别当成你能直接 `workflow('bughunt')` 调起来的命令。
...
**一条铁律，全程适用。** 本书的接地分级里（见 `_grounding.md` A2），关于这五个早期内置 workflow，**被实测确认过的只有「它们曾在注册表里存在」这一层**，也就是 v2.1.150 那行报错列出的五个名字（而到 v2.1.156，这五个里只剩 `deep-research` 还在）。它们的**内部架构（finder 池怎么繁殖、几票算确认、judge 怎么打分）从来没有官方工具定义、也没法逐行读源码、更没经本书复现**。所以下文每一处对「内部怎么跑」的拆解，依据都只有**那一行注册简介加通用模式知识**，标注为「**基于官方 skill 描述 + 行为观察，非源码逐行**」。可以用它来建立直觉、提炼骨架、作为自建蓝本，但不要把其中的数字（3 rapid、5 票、4 评委）当成已经验证过的实现事实，也不要期望今天还能调起 `bughunt`/`plan-hunter`/`review-branch`。下面给的骨架代码也一样，全是**本书的推测性示例实现**，标着「（示意，未实跑）」。
...
**它解决什么问题**：对当前分支做一次「不知道有几个 bug」的全量扫荡，重头戏是**怎么找全**（自繁殖的猎手池）和**怎么信得过**（五票对抗证伪）。
...
- **五票对抗证伪 + pigeonhole 早退**。`5-vote adversarial verification with pigeonhole early-exit`：每条疑似 bug 派几个独立验证者，默认证伪，计票收口；一旦多数已经锁定胜负就提前判决，不再为已成定局的结果付满额成本。这是第 17 章对抗验证加第 15 章 pigeonhole 的合体。
- **综合（synthesis）**收口。
...
骨架的三个齿轮分工明确：**finder 池**负责找（每轮注入已确认清单、只收新增），**对抗证伪管道**负责筛，**`while` + 计数器**负责决定「何时停止」。这是标准的 JavaScript 控制流，模型只管判断，编排交给代码。
...
**它解决什么问题**：`bughunt` 的轻量版，同样是「finder 池 → 五票对抗证伪 → 综合」，但 finder 池是**固定**的（3 rapid + 2 deep，跑完即止），**砍掉了自繁殖和 dry-streak**。

**它示范了哪个模式**（基于官方 skill 描述 + 行为观察，非源码逐行）：**固定 finder 池**。与 `bughunt` 对比，正好演示了一个重要的工程取舍。
...
设计启发在于：**先把验证层（证伪 + 综合）做稳，再把发现层做成可切换的，低风险场景用固定池，高危场景才上自繁殖。** 验证骨架照样复用，发现策略随时可换。
...
**它解决什么问题**：深度调研的脚手架。把一个问题**并行分发（fan-out）**成多路并发检索，抓原文，**对每条论断做对抗式核实**，最后产出带引用的报告。
...
关键纪律：**检索 agent 管「搜和抓」，核实 agent 管「这条到底站不站得住」，两个分开**，别让同一个 agent 既找证据又替自己背书（确认偏误）。注意网络/抓取这类副作用只能塞进 `agent()` 叶子（subagent 才有 Read/Bash/MCP 能力，脚本本身没有 `fetch`）。
...
**它解决什么问题**：穷尽式规划。从**四个不同视角**各出一份独立草案，用**四个并发评委**打分投票选出冠军，再把亚军里的好点子**嫁接（graft）**进冠军，综合出终稿。
...
两条工程纪律：**①「独立」得靠并发加不同的提示词框架来做**（四个 lens 各跑各的，别让它们瞧见彼此）；**②计票用代码、嫁接用 agent**：「谁票多」是确定性运算（`tallyVotes`），「怎么把亮点缝进去」才是判断，交给 agent。
...
#### 5. `review-branch` —— 六维审查 + 逐条对抗验证（早期内置，现已下架）
...
**它解决什么问题**：对当前分支做**六维审查**（bug、简洁性、架构、死代码、推荐做法、模式一致性），**每一条发现在上报前都先过一遍对抗验证**。

**它示范了哪个模式**（基于官方 skill 描述 + 行为观察，非源码逐行）：

- **六维分工审查**。`bugs, simplicity, architecture, dead code, best practices, and pattern consistency` 列了六个**正交的审查视角**，这正是本书第 10 章「分片/多角度审查」的思路：与其让一个 agent「把所有问题一把抓」（注意力摊薄、维度互相干扰），不如一个维度派一个专职 agent，各管一摊。
- **逐条对抗验证**。`Each finding is adversarially verified before reporting`，和 `bughunt` 同源的那套「默认证伪、挺过才上报」，把审查里的假阳性滤掉。

**你能偷师什么**：「六维 fan-out → 汇流 → 逐条对抗验证」这套审查骨架。
...
启发：**审查任务最常见的失误是「一个 agent 包揽所有维度」**。把维度拆开 fan-out，每个维度的 agent 注意力集中、提示词专一，召回率显著提升；再用对抗验证把准确率拉回来。这正是本书第 11 章 PR 多角度 review 的真实做法（Run `wf_4c5caabb-b73`，4 agent，把 26 个问题收敛到 16 个），以及第 10 章分片审查。

### 横向看：五个早期范本共享的同一套「质量套路」

把这五个范本摆一块儿看（不管它们今天还在不在注册表），你会发现它们其实是**同一套质量哲学的五种用法**，不是五个各管各的工具。这套套路有三个固定动作：

```mermaid
flowchart LR
  A["① 独立多视角<br/>(fan-out / 多 lens / 六维 / finder 池)"] --> B["② 对抗验证<br/>(默认证伪 / 评委投票 / 逐条核实)"]
...
| `review-branch`（早期内置，已下架） | 六维分工审查 | 逐条对抗验证 | 汇流上报 |

为什么这套方法反复出现？因为单个 agent 有两个固有缺陷：**①视野窄**（一个 agent 无法覆盖所有角度，必然有遗漏）；**②倾向于编造**（它有一种「总要报告点什么/替自己背书」的倾向，必然产生假阳性）。这三个动作分别应对这两个缺陷、最后收口：

1. **独立多视角治「漏」**：让多个 agent 从互不相同的角度并发出击（`parallel`），最大化召回率。「独立」二字是关键，它们不能互相参考，否则趋同，多出的成本就浪费了。
2. **对抗验证治「编」**：对每一条产出，派出**默认唱反调**的验证者（或评委团），把举证责任压到「这是真的」一方，沉默和犹豫都算「不作数」，假阳性由此被滤掉。
3. **综合收口求「准又省」**：计票、去重、嫁接等**确定性操作交给代码**，只有「如何把亮点整合进去」这类判断才交给 agent。

<div class="callout tip">

**这就是你能从这五个范本里偷到的最值钱的一课：质量是靠「结构」逼出来的，不是靠「让 agent 更卖力」堆出来的。** 你设计任何一个「要可信产出」的 workflow，都可以照这三步搭骨架：先想清楚**哪些视角要并发覆盖**（①），再想清楚**用什么对抗机制过滤假阳性**（②），最后**把确定性收口交给代码**（③）。`agent()` 管判断，`parallel()`/`pipeline()` 管编排，schema 管约束形状，普通 JavaScript 管计票和控制流。这五个范本全是这套积木的不同拼法（哪怕其中四个已从注册表下架，骨架依旧照搬不误）。
...
- **四个案例的归宿**：superpowers 两段式评审落成 `pipeline` 两 stage 加两个 `pass` 门控 schema（不过就有界重来）；OMC Stop 钩子落成 `while(!accepted)` 加验收 schema（钩子和状态文件直接坍缩掉）；ccg 磁盘面包屑落成 structured output 显式传递（在闭包里变成变量，磁盘白白消失）；OmO 工具层护栏落成 `additionalProperties:false` 的 planner schema 加独立 executor 阶段（把「拦截工具调用」往前挪成「约束产物形状」），Category 委派落成 `MODEL_BY_CATEGORY` 映射表。
- **常见发现**：有些「精华」在原生 Workflow 里**无需额外实现**（比如 ccg 面包屑对抗的上下文压缩，在单脚本闭包里根本不存在）。识别出「它是免费获得的」和「移植它」同等重要。
...
下一章，我们把这些验证过的 Workflow 整理成一个**可命名、可参数化、可版本管理、可回归测试、可分享**的个人库。
