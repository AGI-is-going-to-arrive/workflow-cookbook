...
**永远不要写一个只靠模型判决来退出的无界循环。** 模型给的「done」是个概率性判断，它可能因为「想表现得彻底点」而迟迟不肯把 done 置真。`_grounding.md` 还兜了一个全局底：单工作流生命周期里 agent 总数上限 **1000**——这是最后那张安全网，但你**绝不该**指望它来终止业务循环。正确的纪律是：**每个循环都把退出条件显式写出来（轮次上限 + 收益递减），把 budget 当成最后一道防线，而不是唯一防线。**
...
**成本直觉**：循环到干的 token 成本，大致等于「轮数 × 每轮（生成 + 批评两个 agent）」。拿真实数据估一下，单个 agent 约 2.6 万 token（hello `wf_dacbd480-d5d`），那一轮约 5 万、跑 4 轮约 20 万 token。注意这「20 万」是**按单轮估算外推**的数量级，不是实测值；用来对照的 `158982` 才是**实测的 pipeline-demo** 总量——两者不同源，别当成同一次实跑的数据。但它俩落在同一个量级上，足够给你成本直觉。所以轮次上限不只是防死循环，它还是**成本闸门**：把它卡在「边际收益已经很低」的那个轮数上（经验上，3–6 轮足够覆盖绝大多数发散任务）。
