...
为什么要这么较真？网上搜「Workflow API 细节」，搜到的内容里相当一部分都出自同一个第三方视频配套仓库，它不是官方产物。里头有些精确数字（错误类名、超时毫秒、重试次数等）此前在本机无法 runtime 复现。R10 已用 2.1.154 官方二进制核查证实了其中一批（见 [A.14](#a14-第三方未核实清单谨慎)），那属于「二进制确认」，强于第三方声称，但仍不是 runtime 实测。把未核实的内容跟官方的、实测的混在一起写，等于在制造看似权威、实则缺乏根据的噪声。因此本附录宁可少写，也不把未核实的东西当真值。
...
这是你调 Workflow 工具时传进去的入参。脚本从哪来有三种途径（`script` / `name` / `scriptPath`），再加一个 `args`。**`scriptPath` 优先级最高**，高于 `script` 和 `name`。至于 `script` 和 `name` 的相对优先级，官方没有说明，不要假设存在「`scriptPath` > `script` > `name`」这种三级排序。

| 字段 | 类型 | 说明 |
|---|---|---|
| `script` | `string?` | 自包含脚本。**必须**以静态字面量（static literal） `export const meta = {…}` 开头，随后是脚本主体。 |
| `name` | `string?` | 预定义/具名工作流（内置或 `.claude/workflows/`），解析为一段自包含脚本。 |
| `args` | `object?` | 暴露给脚本的全局 `args`。用于参数化具名工作流。 |
| `scriptPath` | `string?` | 磁盘脚本路径。每次调用脚本都会写入磁盘并在结果里返回该路径；可用 Write/Edit 改后用此字段重跑，无需重发整段脚本。**优先级最高**。 |
| `resumeFromRunId` | `string?` | 从某次运行断点续传。未改动的 `agent()` 调用返回缓存结果；**仅同会话**。续传前先停掉上一次运行（TaskStop）。 |

### 持久化-编辑循环：`scriptPath` 的真正威力

`script` 和 `scriptPath` 表面上只是「内联 vs 路径」的区别，但后者实际上支撑了一套完整的迭代流程。【官方】每次跑脚本，无论通过 `script` 还是 `scriptPath` 提交，Workflow 工具都会把脚本**保存到磁盘**，并在返回的 `WorkflowOutput.scriptPath` 里给出保存路径。基于此，迭代循环如下：

```text
首次：用 script 提交  ──►  返回 scriptPath（脚本已写入磁盘）
                              │
            用 Edit 改这个文件 ◄──┘
                              │
...
再配上 `resumeFromRunId`，这套循环就成了「改一处、只重跑改动之后那几步」的底子（见 [A.10](#a10-续传-resume官方实测) 和 [附录 B](#/zh/app-b)）。
...
| `scriptPath` | `string?` | 本次写入磁盘的脚本路径，可 Write/Edit 后作 `scriptPath` 重跑。 |
...
工作流的**真正结果**，是等它跑完时通过 `<task-notification>` 送过来的，里面带着返回值和用量统计（`agent_count` / `tool_uses` / `total_tokens` / `duration_ms`）。`taskId`/`runId` 相当于快递单号，notification 相当于包裹，两者不要混淆。
...
```javascript
export const meta = { /* 静态字面量，见 A.4 */ }
// ↑ 必须第一条语句；以下是脚本主体（async 上下文，可直接 await）
phase('...')
...
- 脚本主体运行在 `async` 上下文，直接 `await`。【官方】
- 标准 JS 内置（`JSON` / `Math` / `Array` / …）可用。【官方】【实测】`Math.max(...)`、`JSON.stringify(...)` 在 `wf_59bf3654-183` 里正常工作。
- **没有**文件系统 / Node API：脚本主体里 `require` / `process` / `fetch` 全是 `undefined`【实测，`wf_59bf3654-183`】。文件、shell、网络这些活只能塞进 `agent()` 叶子里去做，因为只有 subagent 才带 Read/Write/Bash 这些工具。
- **禁用 `Date.now()` / `Math.random()` / 无参 `new Date()`**：它们会破坏续传要靠的可重放性，所以有双层拦着（见 [A.9](#a9-确定性沙箱双层防护实测)）。`new Date(具体值)` 是正常的【实测】。

### 脚本主体内注入的全局函数与对象

下表列出了脚本主体里无需 `import` 即可直接使用的全局。其中 `agent` / `pipeline` / `parallel` / `phase` / `log` / `budget` / `args` / `workflow` 是**官方工具定义**里列出的；`console` / `setTimeout` / `clearTimeout` 则是本书**实测确认**也一并注入的。
...
**说说 `console` / `setTimeout` / `clearTimeout`**：在 `wf_59bf3654-183`（一个 0 agent 的自省工作流）里，`typeof console === 'object'`、`typeof setTimeout === 'function'`、`typeof clearTimeout === 'function'`，而且 `console.log(...)` 也调用成功了（输出进 workflow 日志）。这三个全局确实存在，是实测出来的事实【实测，`wf_59bf3654-183`】。不过日常报告进度还是优先用官方的 `log()`，`console.log` 更像是个调试用的旁路。还有一条相关的实测事实：**VM 给同步执行设了 30000ms 超时**【实测，`wf_e3b2b123-5f4`】。一个不带 `await` 的长同步循环被掐断了，工作流 **failed**（实测耗时 30,222ms），报错一字不差是 `Error: Script execution timed out after 30000ms`。它管的是**同步**这块，专门用来抓死循环，并不是wall-clock上限：带 `await agent(...)` 的异步工作流照样能跑上好几分钟。细节见 [A.14](#a14-第三方未核实清单谨慎) 顶部那张实测升级表。

</div>

---

## A.4 `meta`（导出常量，静态字面量）【官方】
...
**约束**【官方】：`meta` 必须是静态字面量（static literal），不得有变量、函数调用、展开运算符、模板插值（运行时在跑脚本之前会**静态读取**它）。这一条，本书实测的那个第三方校验器（`validate-workflow.mjs`）也逐条查了（见 [附录 B](#/zh/app-b)）。
...
根本原因在于，跑这个探针的会话设了环境变量 `CLAUDE_CODE_SUBAGENT_MODEL`，它**覆盖一切 per-call model**。在 `wf_9c94951d-58c`（**早期 4.7 会话**，`CLAUDE_CODE_SUBAGENT_MODEL=claude-opus-4-7[1m]`）里，5 个分别标了 `haiku` / `inherit` / `opus` / 省略 / 「在标了 `model:'haiku'` 的阶段内」的 agent，**全部跑成了 Opus**。所以这一会话里，本书**无法**把 `phases[].model` 和 `opts.model` 各自的效果拆开来看。（另一条独立环境事实：R11 复核会话的该变量是 `claude-opus-4-8[1m]`，即 Opus 4.8，见 [附录 E · E.2](#/zh/app-e)；本节这个覆盖结论与具体型号无关，4.7、4.8 两个会话都成立。）

<div class="callout warn">

**安全做法**：真正决定模型的，只有 `agent()` 的 `opts.model`。要让某阶段跑 Haiku，就在该阶段的每个 `agent()` 上写 `model: 'haiku'`。`phases[].model` 应视为权限对话框里的展示标签，不要依赖它独立生效。如果环境（或 CI）设了 `CLAUDE_CODE_SUBAGENT_MODEL`，**脚本里所有 `model` 选项都会被静默忽略**，这是用户/CI 层的控制开关，脚本无法干预。实测还有**第二层**覆盖：`ANTHROPIC_DEFAULT_HAIKU_MODEL` / `SONNET` / `OPUS` 会把**模型别名**整体重映射（本会话这两层都指向 Opus，因此脚本里写 `model: 'haiku'` 实跑仍是 Opus，`wf_e8cb23ff-829`）。排查「指定的模型没有生效」时，需要同时检查这两类环境变量。
...
派出一个 subagent 执行任务。这是工作流里唯一会真正消耗 token 的原语：纯编排、不调 `agent()`，则 0 token（`wf_59bf3654-183`、`wf_2b04881f-6a9` 均为 0 token / 个位数毫秒）。
...
### `agentType` 有校验，`model` 没有：一处关键的不对称【实测】

这是本书实测确认的一处关键差异：

- **`agentType` 有校验**【实测，`wf_a222f20f-0f5`】：传入一个不存在的类型，会在**生成任何模型之前**（0 token / 4ms）抛错，并列出全部可用 agent。报错原文如下。
...
> 所以 `agentType` 拼错了就是快速失败、零成本，这一点对调试特别友好。（注：默认 subagent 的类型名叫 `workflow-subagent`，记在每个 agent 的 `agent-<id>.meta.json` 旁车文件里。）

- **`opts.model` 无提交/解析期校验**【实测，`wf_dace2fc6-966`】：传入一个显然不存在的字符串 `'totally-not-a-real-model-xyz'`，**不会在提交/解析期被拒**，agent 照样运行（本会话因为 `CLAUDE_CODE_SUBAGENT_MODEL` 覆盖，实跑 Opus）。与 `agentType` 形成对比：`agentType` 快速失败、零成本，而 `model` 不在解析期拦截无效值。
- 有两点本书**未能核实**：`'inherit'` 字面量的**精确语义**，以及「拼错（比如 `'hauku'`）会先 passthrough、到 **API 调用时**才失败」这一步**（社区第三方资料声称，本书未独立实测）**。后者之所以未能测到，是因为本会话的 `CLAUDE_CODE_SUBAGENT_MODEL` 覆盖了每个 per-call model，无效字符串从未真正发到 API，因此 API 层的失败无法观测。稳妥建议：`model` 只填经过确认的值（如 `'haiku'`，或省略），不要依赖运行时对拼写错误的容错。

---

## A.6 `pipeline(items, stage1, stage2, …) → Promise<any[]>`【官方】

每个 item 各自**独立**地流过全部 stage，**阶段之间没有屏障**。所以wall-clock 时间 ≈ 最慢的那一条单链，而不是各阶段最慢的加起来。**多阶段就默认用 `pipeline()`。**
...
把一组 **thunk**（`() => Promise`）并发执行，**屏障**：等全部完成。结果顺序 = 输入顺序。仅在确实需要所有结果齐备时才使用。

- 异步 reject 或内部 `agent()` 出错，对应位置变为 `null`，调用本身不 reject（使用前先 `.filter(Boolean)`）。但 **thunk 体内同步 `throw`** 的行为不同：它会**reject 整个 `parallel()` 调用**，`.filter(Boolean)` 无法处理这种情况，因为根本拿不到数组。在 `await parallel(...)` 外层套 `try/catch` 可以捕获该异常，**工作流仍然存活**；只有**未被捕获**的同步 throw 才会导致整个运行中止。
...
传给 `parallel()` 的必须是**函数数组（thunk）**（`() => agent(...)`），不能是 Promise 数组（`agent(...)`）。后者会在构造数组的那一刻**立即开始执行**，既不符合 `parallel(thunks)` 的 API，也丢失了异步失败归集语义（async reject 或 agent 出错时，对应位置返回 `null`）。注意：并发上限按**每工作流**计算（`min(16, 核心−2)`），不是 `parallel()` 专属的，不要将此限制误读为「可以绕过运行时节流」。
...
- 这是**硬上限**：`spent()` 达到 `total` 后，再调 `agent()` 会抛错。该池由主循环 + 所有工作流（含嵌套）**共享**。
- 动态循环里必须加 `budget.total &&` 守卫，否则可能持续派发 agent 直到撞上限。

### `workflow(nameOrRef, args?)`【官方】【实测】

内联运行另一个工作流（具名的，或提供 `{ scriptPath }`）。它与主工作流**共享**并发上限 / agent 计数 / 中止信号 / token 预算。本书实测：
...
- **嵌套仅一层**：在子工作流里再调 `workflow()` 就抛错，原文如下。
...
禁用 `Date.now()` / `Math.random()` / 无参 `new Date()`，是为了保住续传要靠的那份可重放性。这套禁用为什么存在、怎么破坏可重放性，[第 01 章 §1.2](#/zh/p1-01) 讲透了；本节只补它的**实测证据**：双层拦截各自的报错原文与 Run ID。本书在 `wf_59bf3654-183` 上从头到尾验证过这两层。

**第一层，提交时静态扫描拒绝**：脚本里只要出现**字面量** `Date.now()`（或 `Math.random()` / 无参 `new Date()`），**提交时**就被静态源扫描拦下，脚本**根本不运行**（返回 `error`，无 Run ID）。你拿 `try/catch` 也救不了，它在解析之前就把你截住了。报错原文如下。
...
> `Math.random` 的运行时报错甚至**直接给出了替代方案**：想要 N 个独立样本，就把下标编进 agent 的 label 或 prompt。这正是确定性的正确做法：同样的脚本 + 同样的 args 必须每次产出同样的结果。
...
那个第三方仓库还捎带一个提交前 lint（`validate-workflow.mjs`），它的行为本书**实跑确认**过（合法脚本会得到 `ok … passes`，违规脚本则逐条报错）。它查的那些规则（meta 须为首条静态字面量语句、禁用非确定性调用、宿主 API 警告、`parallel` 须传 thunk），都以官方工具定义加本书实测为准，校验器无非是把这些规则做成了一个能跑的 lint 而已。详见 [附录 B](#/zh/app-b)。
...
**实测**（`wf_9c94951d-58c`）：头一次跑 5 个 agent = **133,691 token / 32,959ms**；带着 `{ scriptPath, resumeFromRunId }` **原样重跑一遍**，得到同一个 Run ID、5 个结果一模一样、**0 新 token / 3ms**。也就是说，没改动的续传就是「100% 缓存命中」，几乎不花钱。
...
- **`prompt` 入键【实测】**：只改它的 `prompt`（label 还原回去），91,044 重跑成 **60,702 token**（≈基线的 2/3），改动点之前的 agent 还是命中，该 agent 及其下游才重跑。这就是 `label` 那次的正向对照，证明 resume 对内容敏感，并不是随便你改什么它都返回 0。

至于**剩下那些字段进不进键**，也就是「`schema` / `model` / `isolation` / `agentType` 入键、`phase` 不入键」这套说法，**（社区第三方资料声称，本书未独立核实）**：这几个字段本书还没一个个隔离测过。详见 [A.14](#a14-第三方未核实清单谨慎)。
...
**第一层 · 能用（Workflow 工具在不在工具箱里）**。这一层本身又有「官方」和「底层」两副面孔，**官方入口优先**。
...
下表对齐官方文档的 **"Behavior and limits"** 表（信源 `code.claude.com/docs/en/workflows`），并补上本书从 input-schema / 实测拿到的两条（脚本主体积、嵌套层数）：

| 限制 | 值 | 口径 |
|---|---|---|
| 运行中插入用户输入 | **不行**。一个 run 跑起来后中途**不能**塞用户输入；**只有 agent 的权限提示能暂停它**。需要阶段间签收，就把每个阶段拆成**独立 workflow** | 【官方】 |
| 脚本对文件系统 / shell 的访问 | **脚本本身没有**。读写文件、跑命令全由 **agent** 干，脚本只负责编排（这也解释了 A.3「脚本主体里 `require`/`process`/`fetch` 全 `undefined`」那条实测） | 【官方】（+ [A.3](#a3-脚本结构与执行环境) 实测佐证） |
...
| 脚本主体积上限 | **524288 字节（512KB）**（input-schema 的 `script.maxLength`） | 【官方】 |
...
> 结论：**不要把「自带具名工作流」视为稳定 API 面**。v2.1.156 可以稳定依赖的内置只有 `deep-research`，其余需要自行编写或放入 `.claude/workflows/` 自行管理。

---

## A.14 第三方·未核实清单（谨慎）

下面这些说法，最初都出自第三方仓库 `claude-code-workflow-creator`（某 YouTuber 的视频配套仓库，**非官方**）。本书逐条核查后做了**证据分层**：有几条已在本机**实测复现**（R4，见下表），有几条用 2.1.154 官方**二进制核查确认**（R10，强于第三方声称、但仍非 runtime 实测），**其余确实仍无法在本机触发/隔离、未能核实**。**仍未核实的条目，在其他来源中遇到时需要保持警惕，不应视为权威真值。**
