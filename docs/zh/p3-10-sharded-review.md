...
> 大代码库塞不进单个 agent 的上下文窗口。分片审查把目标切成小片，每片派一个 agent 独立审，审完对抗验证，最后综合。本章重点不是「能不能切」，而是怎么用 `pipeline` 让每片独立流过「审查→验证」且阶段间不设屏障，以及什么时候该破例加屏障。它和第 11 章「PR 多维 Review」是孪生：第 11 章按**维度**切（a11y、性能、正确性），本章按**分片**切（文件、模块、函数块）。第 11 章讲「同一份代码、多个视角、屏障收口」，本章讲「多个分片、各自独立、流水线推进」。
...
分片审查利用的正是 Workflow 里 subagent 的独立上下文（见第 06 章）。每片只看自己那一小块，注意力集中；主循环的上下文也不会被原始代码淹没，因为回流的只有结构化的发现。
...
上图把 Review 和 Verify 画成两道「全宽」横条，容易产生误解：以为它们之间有屏障，以为「要等所有分片都 review 完，才一起进 verify」。实际上，分片审查wall-clock最短的形态是：**每个分片审完立刻验，不等别片**，只有最后的 Synthesize 才真的需要看到全部结果。下一节用 `pipeline` 把这个形态写出来。
...
`pipeline(shards, reviewStage, verifyStage)` 的结构与第 08 章的 `pipeline-demo`（Find→Verify）是**同一个模式**，只是 item 从「bug 类别」换成了「代码分片」：
...
- **第二个 stage**（`verifyStage`）拿到 `(review, shard, index)`。`review` 是上一阶段返回的发现集，`shard` 是**原始分片路径**，不需要手动穿线，pipeline 会自动把 `originalItem` 传回来。它对这片里的每条发现并发跑一个验证 agent。

`pipeline` 的语义保证了一件事：`src/auth.ts` 一审完，它的 verify 立刻开始，根本不用等 `src/checkout.ts` 审完。这就是「阶段间无屏障」：A 片可能已经在 Verify 了，B 片还在 Review。

### 嵌套：stage 内部用 `parallel` 验证「该片的多条发现」

第二个 stage 里出现了 `parallel(...)`。这是 `pipeline` 套 `parallel` 的典型组合，需要分清**两层并发各自的边界**：
...
**为什么内层屏障是对的、外层屏障却是错的？** 内层 `parallel` 的屏障只锁「**单个分片**的那几条发现」，粒度小、wall-clock短，而且下一步（过滤出 `real` 的发现）确实得拿到这片的全部裁决。外层要是也上屏障（把 Review 和 Verify 拆成两段全宽 `parallel`），就锁住了**所有分片**，快的分片只能等待最慢那片审完才能开始验证。判据还是第 08 章那条：屏障只在「下一步需要这一组的全部结果」时才用，内层满足，外层不满足（详见第 08 章 §8.5）。

</div>

### `opts.phase` 显式归组，避免并发竞争全局 `phase()`

骨架里每个 `agent()` 都带了 `phase: 'Review'` 或 `phase: 'Verify'`。在 `pipeline`/`parallel` 内部，多个 agent 一起并发推进，如果都靠全局 `phase('Review')` 来归组，进度树就会**竞争错位**（A 片的 verify 可能被归入 B 片刚切出来的 Review 组）。显式写 `opts.phase`，将每个 agent 固定在它所属的进度组里。这是第 05/08 章反复强调的进度归组惯用法。

---

## 10.3 pipeline 分片 vs parallel 屏障：本章的核心取舍

这是分片审查最该想清楚的一道选择题，也是它和第 11 章分道扬镳的地方。两种写法都能跑，但wall-clock代价差得很多。
...
差别在于：如果 `cart` 这片特别大、review 特别慢，写法 A 里 `auth` 和 `checkout` 虽然早已审完、本可立刻开验，也只能等待 `cart` 完成，屏障把快的拖慢到最慢的节奏。写法 B 的 `pipeline` 让 `auth`、`checkout` 一审完就立刻验，wall-clock约等于**最慢的单条链**（`cart` 的 review 加 verify），而不是各阶段最慢之和。

<div class="callout info">

**官方判据（来自工具定义）**：**多阶段任务默认用 `pipeline()`。** 只有当「第 N 阶段需要前一阶段**全部** item 的结果」时，才用屏障（`parallel`）。分片审查的 Review→Verify **不**满足这个条件（验证 auth 的发现，用不着 cart 的发现），所以默认就该用 pipeline。这就是骨架那样写的根本原因（这条判据的完整推导见第 08 章）。

</div>

### 真实数据印证「无屏障」的形态

我们没有专门为分片审查跑一次 N 片的 pipeline，但第 08 章的 **pipeline-demo** 已经确认了这个机制，它就是 Review→Verify 的最小真实形态：
...
`agent_count=6` 精确印证了「3 项 × 2 阶段 = 6 agent」。每项独立流过两阶段、阶段间无屏障，把它原样放大到 N 个分片，就是本章的骨架。把分片数从 3 提到 20，agent 数线性涨到 40，但**wall-clock不会**跟着线性涨：并发上限是 `min(16, 核心−2)`（工具契约与官方类型定义口径，见第 08 章 §8.6），任意时刻最多这么多 agent 在跑，多出来的先排队，有槽位释放再补上，pipeline 又让早完成的分片不空等。
...
- 排优先级这事是全局的：auth 的一个 CRITICAL 必须排在 cart 的一个 LOW 前面，单片 agent 只看得见自己负责的部分，排不出全局序。
...
分片审查的成本几乎可以**事前全部估出来**，这是 Workflow 相比「黑盒 agent」的一大优势。这里用到两条经验法则，它们的完整推导和实测在第 09 章，本章只取结论来应用：**① token 约等于 agent 数乘以每 agent 上下文（约 2.5 万到 3 万 token）；② wall-clock取决于关键路径，即最慢的那条单链，并发把 N 个 agent 压缩到「最慢的一个」的时间。**

应用到分片审查时，先统计 agent 数：
...
以具体例子说明：审查 10 个分片，每片平均产出 4 条发现：
...
代入法则①：`token ≈ 51 × 2.7 万 ≈ 138 万 token`。这个数字不小，但可以提前算出：开跑前就能判断是否会超出本回合的 `budget`（见第 09 章），而不是跑到一半被预算硬上限中断。

<div class="callout warn">

**verify 阶段是 token 大头，不应无条件扩大。** 上例 40 个 verify agent 贡献了近 80% 的 token。如果每条发现都去对抗验证，分片越多、发现越多，verify 的 agent 数就**随分片数与发现数同步快速膨胀**。两个收敛的办法：（a）只验 `high`/`critical` 的发现，在 verify stage 前 `filter(f => ['high','critical'].includes(f.severity))`；（b）让一个 verify agent 一次验**一片的全部发现**，而不是一条配一个 agent，把内层 `parallel` 换成单个带数组 schema 的 agent。后者是用 agent 数换 token，取决于哪种资源更紧缺。

</div>

wall-clock这边套法则②：pipeline 下，10 片的wall-clock约等于**最慢那一片**走完「review → 该片全部 verify →（它在 synthesize 前结束）」的时间，**不是** 51 个 agent 串起来的总和。这就是为什么分片审查能一边看完整个大代码库，一边把 wall-clock 控制在分钟级。真实参照见第 11 章那次 4 维度审查：`agent_count=4` 却跑了约 4.5 分钟（`duration_ms=272643`），因为单 agent 要老老实实读完整个文件，关键路径被「最慢的那个维度」拉长了。
...
**粒度的本质，是「单 agent 上下文」和「agent 数」之间的权衡。** 切得越细（按文件、甚至按函数块），每片上下文越轻、注意力越集中、发现密度越高，代价是 agent 数更多、综合阶段要去重的发现也更多。切得越粗（按目录），agent 更少、综合更省事，代价是单 agent 上下文重，有「读了后面忘前面」的风险。**经验起点是按文件切**：碰到塞不下的超大文件再降到「按函数块/按 region」，碰到文件太多（超过 30 个）再升到「按目录」。

</div>

### Scan 阶段怎么实现？

最简单的 Scan 是**直接传一份文件清单**（如骨架里写死的那个数组，或经 `args` 传进来），零 token、零延迟。需要动态发现时，用一个 agent 配上 `agentType: 'Explore'`（实测可用的内置 agent 之一，见第 06 章）运行 Glob/Grep：
...
注意：文件和 shell 操作只能塞进 `agent()` 叶子里。脚本的主体部分没有 `require`/`process`/`fetch`（宿主 API 缺席，见第 06 章与附录 B），Glob、Grep、Bash 这些都是 subagent 才有的工具。

---

## 10.6 去重与合并：Synthesize 的真正价值

分片审查最后一步「Synthesize」不是把发现拼接在一起就结束，它的核心价值是**去重和排序**，这也正是它需要屏障（看到全部发现）的原因。

**为什么一定会有重复？** 分片是按物理边界（文件、目录）切的，但 bug 不按这个边界分布：

- **同源 bug 跨片复现**：一个有缺陷的工具函数被多片引用，每片审到调用点都报一次。
- **同片多视角命中**：如果分片 agent 的 prompt 包含多个关注点（安全加可读性），同一行代码可能从两个角度各被记录一条。
...
- `shard` 用于**定位回原处**，也让 synthesize 能识别「这两条来自不同片、但描述的是同一个根因 → 合并，并在合并后的 issue 上列出全部命中的 shard」。
...
本书第 11 章的 **frontend-review** 就是一次真实的「分片审查」，只不过它按**维度**分片，不是按文件，对本书自己的 `index.html` 同时从 a11y、性能、正确性三个维度审：
...
2. **发现能直接驱动修复**：那 16 个问题被逐条修进了 `index.html`（XSS、无焦点指示、重复 heading ID 等等）。审查不是终点，而是动手的起点。

<div class="callout info">

**为什么这次真实运行用的是 `parallel`，不是 `pipeline`？** 因为它只有「Review → Synthesize」两阶段，**中间没有 Verify**。Review 完直接进 Synthesize，而 Synthesize 本就需要全部发现，所以三个维度 agent 用一个 `parallel` 屏障并发跑、一起收，是最干净的写法。本章骨架多了一道 **Verify**，正是这道中间阶段，让 `pipeline` 的「无屏障」优势显出来：每片审完立刻验，不用等别片。**有没有中间阶段，决定了你用屏障还是流水线。** 第 11 章会把这次 dogfood 的 26→16 全过程展开。
...
**变体 A · 多文件 × 多维度（pipeline 套 parallel）**：把本章（按文件分片）和第 11 章（按维度分片）叠起来，写成 `pipeline(files, reviewAllDims, synthesizePerFile)`，每个文件独立流过「a11y + 性能 + 正确性并发评审 → 单文件综合」，最后再加一个跨文件的总综合。外层 pipeline 让文件独立流，内层 parallel 让维度并发，这就是两种分片正交叠加的标准形态。
...
- 利用 subagent 的独立上下文：每片注意力集中，主循环只收结构化发现，原始代码不回流，流动的永远是 `{severity, shard, title, fix}` 这样的小对象。
- **核心取舍**：Review→Verify 用 `pipeline`（逐片独立流、无屏障，wall-clock约等于最慢单链），Synthesize 前才用屏障（需要跨片全局视图去重排序）。pipeline 套 parallel 时分清两层屏障：内层（单片多发现）对、外层（跨片）错。
- **给每个发现配上 `severity` + `shard`**：严重度让 synthesize 能全局排序，`shard` 让你能定位回原处、也撑得起跨片语义去重。
- **成本能事前估**：`token ≈ agent 数 × ~2.7 万`，wall-clock取决于关键路径。verify 阶段是 token 大头，按 severity 过滤或「一片一验」来收敛。
- **分片粒度**是「单 agent 上下文」和「agent 数」的权衡：按文件（最均衡）、按目录（少 agent、重上下文）、按维度（正交，第 11 章）、按变更（省 token）。
