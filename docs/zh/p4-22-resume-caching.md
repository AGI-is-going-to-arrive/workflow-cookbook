...
**「什么算改动」——哪些字段会让一个 `agent()` 失去缓存？** 本书实测到的边界是粗粒度的：**同脚本 + 同 args = 100% 命中**（`wf_9c94951d-58c`）——这一条是实打实跑出来的。至于「同一脚本里改动单个 `agent()` 的哪个字段会让它单独失效」，本会话**并未逐一隔离验证**。所以最稳的心智模型是保守的：**只要你改了喂给 agent 的 `prompt`、或它依赖的上游数据，就当它会失去缓存**；要稳吃缓存，就保持脚本与 args 完全不动（只改你确实想重跑的那一步及其下游）。

至于「缓存键到底由哪些字段精确组成」——社区第三方资料声称恰好是 agent 的 `prompt` 加上 `opts` 里的 `schema / model / isolation / agentType`，而 `label`/`phase` 仅作显示、排除在键外。**本书只实测了「同脚本同 args = 100% 命中」，并未逐一验证这份精确组成清单（包括 `label`/`phase` 是否真的不入键）**，故这份「精确字段列表」整体标注为**社区第三方资料声称、本书未独立实测**。实践上你不需要记住精确清单——遵循「改了 prompt/上游数据就当缓存失效」这条保守的粗粒度规则即可安全迭代。
