...
**「什么算改动」——哪些字段会让一个 `agent()` 失去缓存？** 本书实测到的边界是：**同脚本 + 同 args = 100% 命中**（`wf_9c94951d-58c`）——这一条是实打实跑出来的。在此之上，R8 又做了受控实测（基线 `wf_4ffde230-535`，3 个 agent / 91,044 token），单独隔离了两个字段：**只改某 agent 的 `label`（其余不变）→ 续传 0 token 全命中 ⇒ `label` 不入缓存键**；**只改它的 `prompt`（label 还原）→ 91,044 重跑成 60,702 token（≈基线 2/3），改动点之前的 agent 仍命中、该 agent 及其下游重跑 ⇒ `prompt` 入键**。所以最稳的心智模型仍然是保守的：**只要你改了喂给 agent 的 `prompt`、或它依赖的上游数据，就当它会失去缓存**；要稳吃缓存，就保持脚本与 args 完全不动（只改你确实想重跑的那一步及其下游）。

至于缓存键里**其余字段是否入键**——社区第三方资料声称 `opts` 里的 `schema / model / isolation / agentType` 入键、`phase` 仅作显示排除在键外。**本书尚未逐一隔离验证这几个字段**，故它们整体标注为**社区第三方资料声称、本书未独立实测**（已实测的 `label`/`prompt` 见上，不在此列）。实践上你不需要记住精确清单——遵循「改了 prompt/上游数据就当缓存失效」这条保守的粗粒度规则即可安全迭代。
