...
**「仅同会话」是一条硬限制。** 据 `_grounding.md`，断点续传只在**同一个会话**内有效。官方说得很死：**工作流还在跑时你要是退了 Claude Code，下次会话会把这条工作流从头重跑**（原话 *"the next session starts the workflow fresh"*）——没有跨会话续传这回事。说白了，缓存的命就绑在当前会话上，你没法关掉 Claude Code、明天再拿昨天的 `runId` 来续传。所以续传是「**本次迭代会话里**反复调一条流水线」的利器，不是「隔天接着干」的持久化方案。要跨会话保住的状态，得靠别的招（比如让 agent 把产物写到磁盘文件，见第 19 章的控制面/数据面思想）。
