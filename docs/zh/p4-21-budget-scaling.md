...
> 一句话：**`budget` 让你的工作流在运行时『看一眼钱包还剩多少』，据此动态决定扇出多少 agent、要不要降级到便宜模型。它把『花多少 token』从一个赌注，变成一个能算、能自己调的工程量。**
>
...
还有更关键的一点：**这个池是「主循环 + 所有工作流」共享的**（`_grounding.md` B 节）。也就是说，`spent()` 算进去的不只是你这个工作流花的，还有主循环本身、以及同回合起来的其他工作流花的。你不是一个人占着整个预算，而是和别人共用一个钱包，这就让「量入为出」更有必要了。
...
**「会抛错」是官方明确的行为；这个错误的类名（`WorkflowBudgetExceededError`）也已由本书 R10 二进制核查确认。** 「`spent()` 达 `total` 后再调 `agent()` 抛错」来自官方工具定义，可以放心采信；而这个异常的**具体类名** `WorkflowBudgetExceededError`，本书已在 R10 的二进制核查里确认了它确实存在（不再是「第三方声称」）。仍属**本书未实测**的，只剩「预算耗尽时已经在途的 agent 是否跑完、结果保不保留、是不是只是不再启新 agent」这类**精细处置语义**，这一档目前没能独立触发复现。本章不靠它们：所有模式都建立在「**主动量入为出、根本别撞到这个上限**」之上，而不是去 `catch` 某个特定异常类。所以写代码时即便类名可靠，也**别**把控制流押在捕获这个具名异常上，更别假设撞墙后在途结果一定保留。把预算守卫摆在前面，比事后接异常可靠得多。
...
想「量入为出」，先得知道「一个 agent 大概花多少」。本书三组同会话实测（hello / parallel / pipeline，`assets/transcripts/primitives.md`）高度一致地落在 **~2.6 万 token / agent**，据此 `_grounding.md` C 节给出一条经验法则：**token ≈ agent 数 × 每 agent 上下文（约 2.5–3 万 / agent）**。这三组数字的完整推导见[第 09 章 · 进度·日志·续传·预算](#/zh/p2-09)，这里只取它的结论来定价。
...
`files` 有 80 个时，这会一次性排队 80 个 agent（受 `min(16, CPU-2)` 并发上限节流，但**总量**还是 80）。预估成本 80 × 2.6 万 ≈ **208 万 token**。要是用户只给了 `+500k`，跑到第 19 个 agent 左右 `spent()` 就触顶，第 20 个 `agent()` 调用**抛错**，整个工作流中断。到这时你已经为前 19 个白花了钱，连一份完整结果都还没拿到。
...
3. **老实交代 `deferred`。** 因为预算没处理的 item 要如实返回，而不是假装全做完了。这样调用方就能「追加预算、之后再把剩下的跑掉」（甚至用[第 22 章](#/zh/p4-22)的断点续传）。
...
**降级是一种「优雅退化」（graceful degradation）。** 与其在预算告急时硬撑着强模型、跑到一半撞墙崩掉，不如降到 haiku 把**所有** item 都覆盖一遍。一份「全覆盖但精度略低」的结果，往往比「只覆盖一半、但每条都精」更有用。具体怎么取舍，取决于任务：分类、初筛这类适合降级保覆盖；安全审计这种「宁缺毋滥」的就不适合降级。**这个判断应该由你写进代码，而不是让模型临场决定。**

</div>

<div class="callout warn">

**降级要在「扇出前」一次性定下来，别在循环里来回切换。** 如果你在一个长 `pipeline` 里每个 item 都重新 `pickModel()`，可能跑出「前一半强模型、后一半 haiku」这种前后不一致的产物，质量参差、还难以汇总。更稳的做法：**进扇出前先评估一次余量，定下本批次统一的模型策略**；只有在分批、分轮（比如第 18 章的循环）之间才重新评估。</div>
...
**绝不要把 1000 兜底当成业务退出机制（这一点[第 18 章](#/zh/p4-18)讲透了）。** 从规模化的成本角度再补一句：等你真撞到 1000，早就烧掉约 1000 × 2.6 万 ≈ **2600 万 token** 了。真正的规模控制，靠的是你显式的 `budget` 守卫和轮次上限，远在 1000 之前就收住，别让代码走到那一步。
...
把动态扇出、动态降级、边界意识这三件事拧成一个生产骨架：处理一批数量不定的 item，据预算决定**处理多少**、**用什么模型**，最后老实交代结果。
