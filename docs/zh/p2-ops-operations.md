...
**① 关键词触发。** 消息中只要出现 `workflow` 或 `workflows` 一词，Claude Code 会**高亮该词**，表示它接下来会生成一段 workflow 脚本。从这一刻起，Claude 转为「编排脚本」的工作方式，而不是逐回合执行。
...
**误触发了怎么办？按 `alt+w`。** 有时只是顺带提到了「这个 workflow 的设计」，并非要立即运行。此时关键词已经高亮、Claude 也准备生成脚本。按 **`alt+w`** 可以**本次忽略**这个触发，让 Claude 按普通对话处理。这是一个「本次跳过」的快捷开关。
...
一旦关掉，有三件事会立刻变样：**自带命令没了**（比如 `/deep-research` 不再存在）、**`workflow` 关键词失效**（打它不再高亮、也不再把 Claude 切到编排模式）、**`ultracode` 从 `/effort` 菜单里消失**。如果你发现 `/deep-research` 不见了、或关键词不触发了，第一个要查的就是某个关闭开关是不是被打开了。
