...
> 一张**能逐条打勾的清单**。每条告诉你该怎么做、为什么、怎么落地，并链到对应章节。你设计或评审一个 Workflow 时，从头过一遍。勾上的越多，脚本就越确定、越省钱、越容易续传，也越好观察。
>
...
- [ ] **多阶段用 `pipeline()`，不要用 `parallel()` 串联。** ⚠️
  - **为什么**：`parallel` 是个屏障，它要等一批全部跑完才进下一步。你把多阶段写成「`parallel` 接 `parallel`」，每到一个阶段边界就白白等最慢的那一个。`pipeline` 不一样：它让每个 item **独立**流过全部 stage，阶段之间没有屏障，所以wall-clock约等于最慢的那条单链，而不是各阶段最慢的加起来。
...
- [ ] **`parallel()` 只传 thunk（`() => agent(...)`），绝不传 Promise。** ⚠️
  - **为什么**：你传 `agent(...)`（一个 Promise）进去，它在数组刚构造出来时就立即跑了。既不符合 `parallel(thunks)` 的 API，`parallel()` 也没法按 thunk 来管它，还把「async reject / agent 出错就归为 `null`」这套错误归集语义弄丢了。（并发上限的节流是**每工作流**统一施加的，绕不过去。）
  - **落地**：`parallel(items.map(it => () => agent(prompt(it), { schema })))`。详见 [附录 B · B.4](#/zh/app-b)。

- [ ] **`meta` 写成静态字面量（static literal），首行就是 `export const meta = {…}`。** ⚠️
  - **为什么**：运行时在跑脚本之前，会先静态读一遍 `meta`；里面只要带了变量、函数调用、展开或模板插值，就会被拒，工作流根本起不来。
  - **落地**：动态内容挪到 `meta` 之后的代码里，用 `log()`/`phase()` 来表达。见 [第 5 章 · meta 与 phase](#/zh/p2-05)。
...
- [ ] **`parallel`/`pipeline` 内部用 `opts.phase` 显式归组，不靠全局 `phase()`。** ⚠️
  ...
  - **落地**：让每个并发 agent 自己带上 `phase:'Review'`。真实脚本 `frontend-review`/`judge-panel` 都这么干。详见 [附录 B · B.12](#/zh/app-b)。

- [ ] **脚本只做编排，副作用交给 agent。** ⚠️
  - **为什么**：脚本运行在一个受限的 `async` 沙箱里，没有文件系统、没有网络、没有 `require`、也没有 Node 全局。读写文件、联网这些事，都得交给 `agent()` 派出去的 subagent（它们才握着真实工具权限）来做。
...
- [ ] **关键产物用 `schema` 约束形状。** ⚠️
  - **为什么**：带了 `schema`，校验就发生在**工具调用层**：模型给的东西不合规会**自动重试**，直到对上为止，最后返回一个**已验证对象**。下游直接 `.field` 取用就行，不用再去解析自由文本。
...
- [ ] **需要降低单点偏差时，用独立评委 + rubric + 计票。**
  - **为什么**：几个互不通气的评委，对「质量明显分得出高下」的候选能稳定地收敛到一处；rubric（用 schema 把维度固化成数字）加上计票，比「单个 agent 拍脑袋」可靠。真实印证：judge-panel 的 3 名评委独立打分，3:0 收敛（Run `wf_f5b69668-b18`）。
...
- [ ] **保证脚本可重放：禁用 `Date.now()`/`Math.random()`/无参 `new Date()`。** ⚠️
  - **为什么**：这些不确定的来源会破坏续传需要的对齐（运行时也会直接给你抛错）。
...
编排
  ...
  □ parallel 只传 thunk
  □ meta 静态字面量（static literal） + 首行 export
