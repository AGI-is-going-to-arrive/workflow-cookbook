...
> 这份附录把写 Workflow 时最容易踩的坑整理成一张速查表，查得快、对得准。每条都按 **症状 → 原因 → 解法** 写：先说你会看到什么、什么时候出现，再讲清楚底下为什么这样，最后给一段能直接抄走的改法。
>
...
>
> 怎么启用：官方面向用户的入口是 `/config` 里的 "Dynamic workflows" 那一行。它在所有付费档（Pro、Max、Team、Enterprise）以及 Anthropic API、Amazon Bedrock、Google Cloud Vertex AI、Microsoft Foundry 上都能用。Pro 必须在这一行手动打开；其余付费档官方文档没说是否默认打开，所以请到你自己的 `/config` 看那一行的开关状态为准。环境变量 `CLAUDE_CODE_WORKFLOWS=1` 是 power-user 的底层开关，不是官方给的启用方式，也不取代 `/config`（详见 [第 4 章](#/zh/p2-04)）。
...
**症状**：你一调 Workflow 工具，它立刻甩回一个带 `error` 的 `WorkflowOutput`，一个 agent 都没派出去。多半是你把版本号、时间戳，或者某个变量顺手塞进了 `meta`。

</div>

**原因**：运行时在**跑你脚本之前**就先静态读一遍 `meta`，因为它得拿 `name`/`description` 去填那个权限确认弹窗。这一步不会执行你的代码，只做字面量求值。所以 `meta` 必须是**纯字面量**，不能含变量引用、函数调用、展开运算符、模板插值。
...
**原因**：Workflow 在派出任何 subagent 之前，会先给脚本做一遍语法检查。一旦没过，它就把错误写进 `error` 字段、立刻返回，**不消耗任何 agent/token**。这其实是好事，它把写错的脚本拦在花钱之前。
...
**症状**：几个 agent 没被 `parallel()` 统一管起来，它们在数组刚构造的时候就各自先跑了。异步失败归集（async reject 或 agent 出错变 `null`）也跟着失灵。

</div>

**原因**：`parallel()` 收的是**函数数组**（`Array<() => Promise>`，也就是 thunk）。要是你写成 `parallel([ agent(...), agent(...) ])`，那 `agent(...)` 在数组**刚构造出来那一刻就被调用、立刻开跑**了，`parallel` 拿到手的是几个已经在跑的 Promise。这样写有三个后果：它**不符合 `parallel(thunks)` 的 API**；`parallel()` **没法按 thunk 去管这个调用**；它还**丢掉了「单个 thunk 异步 reject 时，该位置归集为 `null`」这套错误语义**，于是你事后 `.filter(Boolean)` 也拦不住它了。

> 顺带说一句：并发上限是 **per-workflow**（整个工作流共享）那道闸，跟你传的是 Promise 还是 thunk 没关系，也不是 `parallel()` 自己能开关的。
...
**症状**：脚本里出现 `Date.now()`、`Math.random()`，或者无参的 `new Date()`，报错指向这三个禁用 API。具体有两种表现。一种是**字面写法**：它在**提交时**就被拒，脚本压根没跑（回执里是 `error` 字段）。另一种是你用别名等动态手段把调用「藏」过了提交期检查，那么**运行时**一调用还是会抛错。

**原因**：这三个都会破坏**可重放性**。续传（`resumeFromRunId`）靠的是「同样的脚本 + 同样的输入 → 同样的执行路径」，这样运行时才能判断哪些 `agent()` 调用没变、可以直接复用缓存。一旦脚本里掺了不确定来源，重放就对不齐了。所以这三个被**两层**拦死。**第一层**是提交期的**源码静态扫描**：只要源码里出现这几个 token 的字面写法，哪怕写在注释、字符串、或永不执行的分支里，整段脚本在运行**前**就被拒，根本进不到执行。**第二层**是**运行时陷阱**：就算你用别名绕过了第一层，这些全局在运行时也已被改造，一调用就抛错。为什么禁得这么死，背后的确定性原理见 [第 01 章 §1.2](#/zh/p1-01)；两层的完整机理、报错原文、以及绕过和接住的误区见 [B.19](#b19-trycatch-接不住-datenow)。
...
**原因**：嵌套**只允许一层**。父调子可以，子再调孙就抛错。这是故意设的护栏，防着递归失控：子流程跟父流程共用并发上限、agent 计数、中止信号和 token 预算，无限往下嵌套会让这些共享资源失去边界。
...
**原因**：`schema` 的校验是在**工具调用层**做的。模型必须调 `StructuredOutput` 工具，而且输出要严格对上 schema，对不上就**重试**。要是 schema 卡得太死（比如 `enum` 没列全、`required` 里塞了模型其实给不准的字段、数值 `pattern` 或范围太窄），模型可能得试好多次才偶然蒙对，表现出来就是变慢，甚至快要放弃。
...
**原因**：**文件写入只有原生 Write/Edit 工具会落盘**。`ctx_execute` 和 Bash 子进程是在子进程里跑的，用完就扔，它们往文件系统里写的东西**不持久化**到宿主机。同样的道理，脚本体自己**没有文件系统 API**，它的产物是 `agent()` 的**返回值**（文本或结构化对象），不是磁盘上的文件。
...
**原因**：Workflow 工具**始终异步**。它立刻甩回一个**回执**（`status` 只会是 `"async_launched"` 或 `"remote_launched"`），工作流则在后台接着跑。**真正的返回值和用量统计**（`agent_count`/`tool_uses`/`total_tokens`/`duration_ms`）是在完成时通过 `<task-notification>` 送过来的。
...
**原因**：脚本体是个**被限制住的 `async` 沙箱**。标准 JS 内置（`JSON`、`Math`、`Array`、`Object`、`Promise` 等等）能用，但**没有**文件系统、网络、`require`、Node 全局这一类能力。工作流的副作用全靠 `agent()` 派出去的 subagent 来干，握着真实工具权限的是它们，不是脚本体。
...
**原因**：这跟 [B.4](#b4-parallel-传了-promise-而非-thunk) 是**姊妹陷阱**。B.4 是「类型传错了（传了 Promise 而非 thunk）」，本条是「thunk 传对了，但 thunk **函数体里同步 `throw`**」。`parallel()` 是一个个**调用** thunk 的。要是 thunk 体里有同步 `throw`（比如裸 `throw`、`JSON.parse` 失败、断言、下标越界），异常会在 `parallel()` 拿到 promise **之前**就往上抛，于是它**没有机会把这一格收集成 `null`**，整个 `parallel()` 调用直接 **reject**。这跟异步失败是两套语义：异步 reject 只把那一格变 `null`，同步 throw 则掀掉整个调用。

**但「调用 reject」不等于「运行必崩」**。在 `await parallel(...)` 外面套一层 `try/catch`，就能**干净接住**它，workflow **照样活着**（实测 `wf_b7c75d40-c26`：`callRejected:true`、`caughtByTryCatch:true`、`runSurvived:true`）；只有**没接住**的同步 throw 才会把整个运行中止。还要注意：工具文档那句「a thunk that throws resolves to null」只对**返回的 promise 异步 reject**成立，对**同步 throw 不成立**。
...
> 为啥管这么严？因为续传要的是「同脚本 + 同输入 → 同执行路径」，任何不确定来源都会把对齐搅黄。两层防护就是要让你**不可能**把不确定性偷渡进一个可重放的脚本里。这套确定性原理在 [第 01 章 §1.2](#/zh/p1-01) 讲得最透。
...
**实测的诚实边界**：本会话里 `CLAUDE_CODE_SUBAGENT_MODEL` 把一切 per-call `model` 都盖掉了（实跑的是 Opus），所以那个 bogus 字符串**从未真正发往 API**。也就是说「拼错会在 API 调用时失败」这一步，本会话**未能观测到**（社区第三方资料如此声称，本书未独立实测）。能拍胸脯确认的只有一条：**提交期不报错**。
...
1. **失败是发生在「启动前」还是「执行中」？** 启动前就看 `error` 字段（`meta` 或语法，[B.2](#b2-meta-非纯字面量被拒)/[B.3](#b3-语法错落进-error-字段)），这不花钱；执行中就看完成通知和 `null`（[B.5](#b5-datenow-mathrandom-抛错)/[B.11](#b11-结果里的-null-没过滤)/[B.15](#b15-pipeline-单项静默掉队)）。
2. **是不是把「沙箱」当成了「主机」？** 脚本体没有 Node API，`ctx_execute` 和子进程的写入也不落盘，所以副作用和文件操作都得过 agent 的真实工具（[B.10](#b10-沙箱写入不落盘的误解)/[B.16](#b16-想在脚本体里用-node-api)）。
...
你写的 workflow 脚本，迟早会被分发到别人机器上跑：可能是同事的 Windows 笔记本，可能是 CI 里的 Linux 容器，也可能是 GitHub Pages 那种大小写敏感的部署环境。**一段在你 Mac 上跑得好好的脚本，换台机器可能就翻车。** 哪些地方会翻、哪些地方稳，得先分清楚。

最要紧的一条直觉：**Workflow 脚本本体跑在一个 JS 沙箱里，而这个沙箱在三个平台上是同一套**。所以脚本逻辑层面的行为（确定性守卫、`args` 怎么传、`require/process/fetch` 缺席、30000ms 同步超时、错误怎么往上抛）**跟你装的是 Windows 还是 Linux 没半点关系**。真正会因平台而异的，全发生在**沙箱之外**：要么是你在 `agent()` 叶子里让 subagent 去碰了文件系统或 shell（那才算碰着真实操作系统），要么是 git worktree、部署环境这些外部设施。
...
**误伤陷阱**：守卫扫的是源码文本，不是真实调用。你在字符串里写 `Date.now()` 本想拿它当文档或提示，也照样会让整个脚本提交失败。要么拆开拼（`'Date'+'.now()'`），要么换个措辞。这跟平台无关，三平台一律拒。
...
**⑤ `meta` 必须纯字面量，外加各种校验。** 证据等级：**【平台无关·已实测】**。`meta` 必须是纯字面量（保留键比如 `constructor` 会被拒）；未知的 `agentType` 在生成模型前就抛错并列出可用 agent；`isolation:'remote'` 被拒，而 `isolation:'totally-bogus'` 被静默忽略。这些校验全在运行时或提交期完成，**与操作系统无关，三平台一致**。机理跟 ① 一样：扫的是脚本结构和取值，不是 OS 行为。
...
- **路径分隔符**：Unix 系（macOS / Linux）用 `/`，Windows 原生用 `\`（虽说多数 API 也认 `/`）。要是你在 prompt 里硬编码 `dir/sub/file.txt` 这种路径让 subagent 去读写，**在 Windows 上稳不稳，取决于 subagent 实际用的工具或 shell 怎么解析**。本书未实测，按通用知识推断这里有风险。建议让 subagent 用相对路径，或者在 prompt 里把话说清楚「用你所在平台的原生路径写法」，别替它去拼绝对路径。
...
**② worktree 隔离需要 git 仓库，非 git 目录下的行为待验证。** 证据等级：**【推断·未实测】**。`opts.isolation:'worktree'` 的机理是「在一个**独立的 git worktree** 里跑这个 agent」（昂贵，约 200–500ms 启动外加磁盘和 agent 开销，仅在并行改文件会冲突时用）。`git worktree` 是 git 的功能，**前提是当前目录得在一个 git 仓库里**。由此**推断**（未实测）：要是你在一个**非 git 目录**（或没初始化 git 的项目）里用 `isolation:'worktree'`，底层的 `git worktree add` 没仓库可挂，**很可能失败或退化**。但具体是抛错、还是静默退化成「不隔离」（类比 B.24.1 ⑤ 里 bogus isolation 被忽略那种行为），**本书没测过，不下结论**。这条跟 OS 关系不大，跟「有没有 git 仓库」关系更大，但它确实是个会让 workflow 在某些环境翻车的 corner case。安全做法：用 `isolation:'worktree'` 的 workflow，文档里写明「需在 git 仓库根目录运行」。
...
**④ 换行符、文件编码（小坑，推断）。** 证据等级：**【推断·未实测】**，属于通用知识，非本特性实测。`agent()` 写出来的文件：Windows 习惯 `CRLF`、Unix 习惯 `LF`，编码上偶尔有 BOM 差异。要是下游 agent 或外部工具对换行、编码敏感（比如某些 diff、某些解析器），**可能**在跨平台时表现不同。这属于通用的文件 IO 知识，**本书未实测**，只作一句提醒，不展开。
