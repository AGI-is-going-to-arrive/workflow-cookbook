...
We unpack each piece below. Every section follows the same structure: **pattern → script (orchestration trade-offs) → real run (Run ID + usage table) → result → teaching point.** The mechanics of the three shapes themselves are covered in [Chapter 08](#/en/p2-08) (pipeline / parallel), [Chapter 17](#/en/p4-17) (adversarial verify), and [Chapter 18](#/en/p4-18) (loop-until-dry); this chapter only shows what they look like in a real run.
...
**security (4)**: all **latent / supply-chain** issues, with no input surface an attacker could reach. They include mermaid SVG injected via `innerHTML` after sanitize (leaning only on `securityLevel:'strict'`), 4 CDN scripts with no SRI, `ghLink.href` with no scheme validation, and inconsistent escaping of manifest fields.
...
The thing to take away from this section is not "found 18 bugs." It is this: **the verify stage didn't just judge true/false; it pulled back the reviewer's exaggerations.** The precision clarifications in `verdict.reason` are themselves a product:
...
Scan a target round by round for symbols that are "defined but never referenced anywhere in the file," **stopping only after several empty rounds in a row.** This is the loop-until-dry shape ([Chapter 18](#/en/p4-18)): use a `while` loop to keep dispatching an agent until it's "dry" (consecutive empty rounds). You can't call it after a single round, because confirming one symbol dead may make another one obviously removable too.
...
**Trade-off 1: two termination conditions (`DRY_STREAK` + `MAX_ROUNDS`).** `emptyRounds < DRY_STREAK` is the normal exit for "stop once dry"; `round < MAX_ROUNDS` is the hard cap of "run at most 5 rounds no matter what." The latter is a safety net. Should the agent report something new every round (even noise), the loop still won't run forever. This echoes the runaway-loop backstop idea in `_grounding.md` ("lifetime `agent()` total cap 1000"): loop-style workflows **must** carry their own hard cap (see [Chapter 18 §18.3](#/en/p4-18)).

**Trade-off 2: report-only, never edits files.** The prompt spells it out: `Report only — do NOT edit any file`. This is the safe posture for a scanning "sweep": report it up first, a human looks, then you decide whether to change anything, rather than turning the agent loose to delete code on its own. Delete dead code wrongly and you can bury a subtle bug, so it stays hands-off by default (cf. Chapters [16](#/en/p3-16), [18](#/en/p4-18)).
...
**Summarize a batch of feedback in parallel**, then cluster the **whole set** into ranked themes. Here's the key: the cluster step **must wait for every summary to arrive** before it can run, since you can't cluster a single piece of feedback on its own. This is exactly the right scenario for `parallel()` as a **barrier** (rather than a pipeline) ([Chapter 08](#/en/p2-08) contrasted the two).

The input is a clearly-labeled **synthetic sample** `assets/samples/feedback-sample.csv` (18 rows, columns `id,text`); but **the run itself is real**, with Run ID, tokens, and clustered output all traceable.
...
**Trade-off 2: `.filter(Boolean)`.** `parallel()`'s semantics are "an agent erroring asynchronously → that slot is `null`, the call itself doesn't reject" (full failure semantics in [Chapter 08 §8.8](#/en/p2-08)). So once you've got `summaries`, you first `.filter(Boolean)` to drop the failed slots, then feed clustering. This is the standard defensive pattern when you use `parallel`.
