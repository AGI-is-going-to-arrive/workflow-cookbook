...
- The official loop's **closer**: once a run you're happy with finishes, press `s` in the `/workflows` view to **save it as a `/` command** for instant reuse — this is the most natural on-ramp for a beginner "building your own workflow" (growing it into a library is [Chapter 25](#/en/p5-25)).
