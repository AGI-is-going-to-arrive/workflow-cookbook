...
> Theory's done. Now you get your hands dirty. This chapter goes from "confirm the environment" to "get your first Workflow running and understood," walking the whole loop of launch, async, progress, and iteration. Every step is checked against **real-run** output.
...
**Two 0-cost confirmation methods.** First: just say it in the conversation, "run a minimal workflow to confirm the runtime." The sentence carries the word `workflow`, so Claude reaches for the Workflow tool. If it's on, it runs; if not, it tells you the tool is unavailable. Second: type `/effort` and check whether the picker has an `ultracode` slot. If it does, workflows are already **available** (the reasoning is in §1.6).
...
**Don't let the word "script" scare you off: you don't hand-write this script, Claude writes it for you.** The entry the official docs designed for ordinary users is a smooth closed loop. You say a need **in plain language** that carries the word `workflow`, e.g., "run a workflow to sweep this repo's TODOs and group them." Claude writes the orchestration script for you. Before it runs you get one approval prompt and a look, and when unsure you hit `View raw script` to read the source. Once it runs and you like it, one keypress **saves it as a `/` command** for instant reuse next time. So the scripts below are here for you to **read and understand**; when you actually do this, Claude writes, you review, you save. How you drive this loop in the terminal (the 4 approval options, pressing `s` to save the command) has a hands-on walkthrough in [The Official Control Panel](#/en/p2-ops); this chapter focuses on "what the script itself looks like, how to read it, how to iterate it."
...
**This is a Workflow script, not a Node script, and it's a beginner's first pothole.** `meta`/`phase`/`agent`/`log`/`budget`/`args` are all globals **injected by the Workflow runtime** (`_grounding.md` section B: "injected at runtime, no import needed"). Save this as `hello.js`, run `node hello.js` on its own, and Node, which has none of these globals, throws `ReferenceError: phase is not defined` right away. **This is identical on Windows, macOS, and Linux**: it has nothing to do with the OS, it's that Node simply has no Workflow runtime layer. It only runs inside a Claude Code session where workflows are available, executed by Claude through the built-in Workflow tool. How to confirm it's available, and how the official path turns it on, are in 4.1 and [Chapter 01 §1.5](#/en/p1-01). Triggering it is simple too: just include the word `workflow` in your message (see 4.1). This book's testing ran it exactly that way: runtime confirmed, schema forced `sum=4` as a **number**, ~26k tokens / ~5.5 seconds (see the real receipt and usage in 4.3 and 4.4).
...
- The simplest agent round-trip is about **5.5 seconds / 26k tokens.** Take that as your baseline unit and you can estimate what a bigger workflow will cost.
...
The script already landed on disk (the receipt's `Script file` / `WorkflowOutput.scriptPath`), so iterating a workflow doesn't mean resending the whole code every time. That gives you an **"edit the on-disk file → re-run with `scriptPath`" iteration loop**:
...
**① `meta` is not a pure literal (including "computing a value inside `meta`").** `meta` must be a "dead" literal. The runtime reads it during the **static-parsing phase**, so any variable reference, function call, spread, or template interpolation makes it refuse to launch. Beginners especially love to "just compute something" in `meta`, like stitching a name together or generating a description from the date, and that's exactly the trap that catches them most:
...
**Don't use `Date.now()` / `Math.random()` / arg-less `new Date()` in scripts.** They break replayability and kill the resume cache (see 4.5). This ban is measured to have **two layers** (commit-time source scan + runtime trap); it catches the literals even inside a comment or a string, and `try/catch` can't save you. The full mechanism of both layers, the verbatim error text, and the misconceptions about bypassing/catching them are gathered in [App B, B.5 / B.19](#/en/app-b), with the causal chain in [Chapter 22](#/en/p4-22). All you need here are the alternatives: need a timestamp? Pass it in via `args`. Need randomness? Vary the prompt using the agent's index.
...
- First make sure workflows are available in your session: they're on **all paid plans**, and on Pro you flip them on from the "Dynamic workflows" row in `/config`. The official docs don't state a default for the other plans, so check that toggle in your own `/config` (three ways to turn them off are in §4.1). `CLAUDE_CODE_WORKFLOWS=1` is just a low-level observation switch, not the official enable path (both layers in [§1.5](#/en/p1-01)). If you're not sure, have Claude run a minimal workflow to confirm.
...
- Real baseline: a single agent is about 5.5s / 26k tokens; `schema` guarantees the return type (`sum` is the number 4, not a string).
- Iterate via the "script is a file" loop: edit the on-disk `.js` + re-run with `scriptPath`; add `resumeFromRunId` to reuse the cache.
- The official loop's **closer**: once a run you're happy with finishes, press `s` in the `/workflows` view to **save it as a `/` command** for instant reuse. For a beginner, this is the most natural on-ramp to "building your own workflow" (growing it into a library is [Chapter 25](#/en/p5-25)).
...
**Build your own workflow · 6 steps.** This through-line is scattered across the book, so here is a single visible table of contents you can just follow:
...
> Once it's running, how do you watch progress, stop it, or press `s` to save a run you like as a `/` command? The whole terminal operating surface is in [The Official Control Panel](#/en/p2-ops).
