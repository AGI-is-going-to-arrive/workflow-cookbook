...
**"What counts as a change" — which fields make an `agent()` lose its cache?** The boundary this book tested is: **same script + same args = 100% hit** (`wf_9c94951d-58c`) — that one is hard-tested. On top of that, R8 ran a controlled test (baseline `wf_4ffde230-535`, 3 agents / 91,044 tokens) that isolated two fields individually: **changing only one agent's `label` (everything else unchanged) → resume is a 0-token full hit ⇒ `label` is not in the cache key**; **changing only its `prompt` (label restored) → 91,044 re-runs as 60,702 tokens (≈2/3 of baseline), with agents before the change point still hit and that agent plus its downstream re-run ⇒ `prompt` is in the key.** So the steadiest mental model is still conservative: **if you change the `prompt` fed to an agent, or the upstream data it depends on, treat it as losing the cache**; to reliably keep cache hits, leave the script and args completely untouched (change only the one step you genuinely want to re-run, plus its downstream).

As for **whether the remaining fields are in the key** — third-party community material claims `schema / model / isolation / agentType` from `opts` are in the key, with `phase` being cosmetic and excluded. **This book has not yet isolated and verified these fields one by one**, so they are marked in their entirety as **claimed by third-party community material, not independently tested by this book** (the already-tested `label`/`prompt` are above, not in this list). In practice you don't need to memorize the exact list — following the conservative coarse rule "changing prompt/upstream data means treat the cache as lost" lets you iterate safely.
